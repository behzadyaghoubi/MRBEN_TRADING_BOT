...
def debug_signal_pipeline():
    ...
    print(f"   Probabilities: SELL={prediction[0][0]:.4f}, HOLD={prediction[0][1]:.4f}, BUY={prediction[0][2]:.4f}")
    ...
    bb_buy = current_price < bb_lower[-1] if len(bb_lower) > 0 and not np.isnan(bb_lower[-1]) else False
    bb_sell = current_price > bb_upper[-1] if len(bb_upper) > 0 and not np.isnan(bb_upper[-1]) else False
    ...
    weighted_confidence = (
        confidence * lstm_weight +
        ta_confidence * ta_weight +
        ml_confidence * ml_weight
    )
    
    print(f"📊 Weighted Confidence Calculation:")
    ...
    lstm_signal = signal_map[signal_class] if signal_class == 2 else (-1 if signal_class == 0 else 0)
    ...
    if lstm_signal == ta_signal and lstm_signal != 0:
        ...
        if ml_confidence > MIN_SIGNAL_CONFIDENCE:
            ...
            print(f"✅ ML filter approves (confidence {ml_confidence:.3f} > {MIN_SIGNAL_CONFIDENCE})")
        else:
            final_signal = 0
            final_confidence = 0.0
            print(f"❌ ML filter rejects (confidence {ml_confidence:.3f} <= {MIN_SIGNAL_CONFIDENCE})")
    else:
        ...
        if ml_confidence < MIN_SIGNAL_CONFIDENCE:
            final_signal = 0
            final_confidence = 0.0
            print(f"❌ ML filter rejects final signal (confidence {ml_confidence:.3f} < {MIN_SIGNAL_CONFIDENCE})")
