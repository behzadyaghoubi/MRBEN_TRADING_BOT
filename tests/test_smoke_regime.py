...
def test_smoke_without_regime():
    ...
    cmd = [sys.executable, "live_trader_clean.py", "smoke", "--minutes", "1", "--symbol", "XAUUSD"]

    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)

        assert result.returncode == 0, "Smoke test without regime failed"
...
def test_smoke_agent_flag():
    ...
    try:
        ...
        assert result.returncode == 0, "Smoke test with agent failed"
