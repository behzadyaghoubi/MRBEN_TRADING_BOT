...
def test_signal_generation():
    ...
    try:
        ...
        from live_trader_clean import MT5SignalGenerator, MT5Config
        ...
        signal_generator = MT5SignalGenerator(
            config, lstm_model, lstm_scaler, ml_filter
        )
        ...
        for i in range(5):
            ...
            assert signal_data['signal'] in [-1, 0, 1], f"Invalid signal value: {signal_data['signal']}"
            assert 0 <= signal_data['confidence'] <= 1, f"Invalid confidence value: {signal_data['confidence']}"
...
def test_feature_extraction():
    ...
    try:
        ...
        print(f"   Expected: 10")
        print(f"   All features are numeric: {all(isinstance(f, (int, float, np.number)) for f in feature_list)}")
        ...
        try:
            ...
            if os.path.exists(ml_filter_path):
                ...
                print(f"✅ ML Filter prediction successful!")
...
def main():
    ...
    if test1_success and test2_success:
        ...
        print("✅ The live_trader_clean.py should now work correctly without feature shape mismatch errors.")
    else:
