...
class MT5Config:
    ...
    def _load_config(self):
        ...
        default_config = {
            ...
            "risk": {"volume": 0.01, "min_signal_confidence": 0.6},
        ...
        if os.path.exists(self.config_path):
            try:
                ...
                # Merge with defaults
                self.SYMBOL = config.get("trading", {}).get(
                    "symbol", default_config["trading"]["symbol"]
                )
                self.TIMEFRAME = config.get("trading", {}).get(
                    "timeframe", default_config["trading"]["timeframe"]
                )
                self.BARS = config.get("trading", {}).get("bars", default_config["trading"]["bars"])
                self.SLEEP_SECONDS = config.get("trading", {}).get(
                    "sleep_seconds", default_config["trading"]["sleep_seconds"]
                )
                self.RETRY_DELAY = config.get("trading", {}).get(
                    "retry_delay", default_config["trading"]["retry_delay"]
                )
                self.CONSECUTIVE_SIGNALS_REQUIRED = config.get("trading", {}).get(
                    "consecutive_signals_required",
                    default_config["trading"]["consecutive_signals_required"],
                )

                self.VOLUME = config.get("risk", {}).get(
                    "fixed_volume", default_config["risk"]["volume"]
                )
                self.MIN_SIGNAL_CONFIDENCE = config.get("ml", {}).get(
                    "accept_threshold", default_config["risk"]["min_signal_confidence"]
                )

                self.LOGS_DIR = (
                    config.get("logging", {})
                    .get("log_file", default_config["logging"]["logs_dir"])
                    .replace("/trading_bot.log", "")
                )
    ...
    def _set_defaults(self, default_config):
        ...
        self.CONSECUTIVE_SIGNALS_REQUIRED = default_config["trading"][
            "consecutive_signals_required"
        ]
...
class MT5SignalGenerator:
    ...
    def _generate_lstm_signal(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            if len(available_features) < 8:
                self.logger.warning(
                    f"Available features: {available_features}, need 8 features for LSTM"
                )
                return {'signal': 0, 'confidence': 0.0}
            ...
            return {'signal': signal, 'confidence': confidence, 'raw_prediction': prediction[0]}
    ...
    def _generate_technical_signal(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            return {'signal': signal, 'confidence': min(confidence, 0.9), 'rsi': rsi, 'macd': macd}
    ...
    def _apply_ml_filter(self, lstm_signal: dict, ta_signal: dict) -> dict:
        ...
        try:
            if self.ml_filter is None:
                ...
                combined_signal = (
                    lstm_signal['signal'] * lstm_weight + ta_signal['signal'] * ta_weight
                )

                combined_confidence = (
                    lstm_signal['confidence'] * lstm_weight + ta_signal['confidence'] * ta_weight
                )
        ...
        try:
            ...
            if os.path.exists(ml_filter_path):
                ...
                ml_filter = AISignalFilter(
                    model_path=ml_filter_path, model_type="joblib", threshold=0.65
                )
            ...
            try:
                ...
                if ENHANCED_AI_AVAILABLE:
                    ...
                    if self.conformal is not None:
                        ...
                        conformal_p = p_hat
                        self.logger.info(
                            f"Conformal: accept={conformal_ok} p={p_hat:.3f} regime={regime}"
                        )
                        if not conformal_ok:
                            time.sleep(self.config.SLEEP_SECONDS)
                            continue
                ...
                should_execute = (
                    signal_data['confidence'] >= adj_thr
                    and self.consecutive_signals >= self.config.CONSECUTIVE_SIGNALS_REQUIRED
    ...
    def _execute_trade(self, signal_data: dict, df: pd.DataFrame):
        ...
        try:
            ...
            if self.tp_policy["split"]:
                ...
                tp2 = entry_price + (
                    side * risk * max(self.tp_policy["tp2_r"], self.min_R_after_round)
                )
