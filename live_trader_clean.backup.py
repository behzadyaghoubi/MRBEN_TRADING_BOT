...
def _modify_position_sltp(position_ticket: int, symbol: str, new_sl: float = None, new_tp: float = None, magic: int = None, deviation: int = 20):
...
class MT5DataManager:
    ...
    def _get_synthetic_data(self, bars: int) -> pd.DataFrame:
        ...
        for i in range(bars):
            ...
            data.append({'time': now - timedelta(minutes=i*self.timeframe_min),
...
class MRBENAdvancedAISystem:
    ...
    def _tech_pred(self, df: pd.DataFrame) -> Dict[str, Any]:
        try:
            if len(df) == 1:
                ...
                if ch > 0.0005: return {'signal': 1, 'confidence': 0.7, 'score': 0.3}
                if ch < -0.0005: return {'signal': -1, 'confidence': 0.7, 'score': -0.3}
            ...
            score = 0.0
            conf = 0.5
            if rsi < 35: score += 0.4; conf += 0.2
            elif rsi < 45: score += 0.2
            elif rsi > 65: score -= 0.4; conf += 0.2
            elif rsi > 55: score -= 0.2
            ms = macd - macds
            if ms > 0.03: score += 0.3; conf += 0.1
            elif ms < -0.03: score -= 0.3; conf += 0.1
    ...
    def _lstm_simple(self, df: pd.DataFrame) -> Dict[str, Any]:
        try:
            if len(df) == 1:
                ch = (df['close'].iloc[0] - df['open'].iloc[0]) / df['open'].iloc[0]
                if ch > 0.0002: return {'signal': 1, 'confidence': 0.75, 'score': 0.3}
                if ch < -0.0002: return {'signal': -1, 'confidence': 0.75, 'score': -0.3}
                return {'signal': 0, 'confidence': 0.65, 'score': 0.0}
            rsi = df['rsi'].iloc[-1] if 'rsi' in df.columns else 50
            if rsi < 40: return {'signal': 1, 'confidence': 0.8, 'score': 0.4}
            if rsi > 60: return {'signal': -1, 'confidence': 0.8, 'score': -0.4}
            return {'signal': 0, 'confidence': 0.6, 'score': 0.0}
        except Exception:
            return {'signal': 0, 'confidence': 0.5, 'score': 0.0}
    ...
    def generate_ensemble_signal(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            score = lstm['score']*self.ensemble_weights[0] + ml['score']*self.ensemble_weights[1] + tech['score']*self.ensemble_weights[2]
            if score > 0.05: sig = 1
            elif score < -0.05: sig = -1
            else: sig = 0

            conf = lstm['confidence']*self.ensemble_weights[0] + ml['confidence']*self.ensemble_weights[1] + tech['confidence']*self.ensemble_weights[2]
            self.logger.info(f"Ensemble: LSTM({lstm['signal']},{lstm['score']:.3f}) ML({ml['signal']},{ml['score']:.3f}) TECH({tech['signal']},{tech['score']:.3f}) => Final({sig},{score:.3f})")
            return {'signal': sig, 'confidence': float(conf), 'score': float(score), 'source': 'Advanced AI Ensemble'}
        except Exception as e:
...
class EnhancedTradeExecutor:
    ...
    def modify_stop_loss(self, symbol: str, position_ticket: int, new_sl: float, current_tp: Optional[float] = None) -> bool:
        try:
            if not MT5_AVAILABLE:
                return False
            ...
            request = {
                ...
                "symbol": symbol,
                "sl": float(adj_sl),
                "tp": float(tp_to_send) if tp_to_send else 0.0,  # اگر واقعاً قصد حذف TP داری، صِفر بده
            }
            result = mt5.order_send(request)
    ...
    def get_account_info(self) -> dict:
        try:
            ...
            if not a:
                return {'balance': 10000.0, 'equity': 10000.0, 'margin': 0.0, 'free_margin': 10000.0}
            return {'balance': a.balance, 'equity': a.equity, 'margin': a.margin, 'free_margin': a.margin_free}
        except Exception:
            return {'balance': 10000.0, 'equity': 10000.0, 'margin': 0.0, 'free_margin': 10000.0}
...
def _append_trade_log_csv(row: dict, path: str):
    ...
    cols = [
        "run_id","timestamp","symbol","action","entry_price","sl_price","tp_price","volume",
        "confidence","source","atr","vol_ratio","sl_mult","tp_mult","spread_price","spread_frac",
        "R_multiple","R_multiple_post_round","fallback_used","tp_policy",
        "mt5_order_id","mt5_retcode","mt5_executed","mt5_error"
    ]
    # تضمین وجود کلیدها
    for c in cols:
...
class MT5LiveTrader:
    def __init__(self):
        ...
        try:
            ...
            fh = RotatingFileHandler(self.config.LOG_FILE, maxBytes=5_000_000, backupCount=5, encoding='utf-8')
            ...
            exists = any(isinstance(h, logging.handlers.RotatingFileHandler) and getattr(h, 'baseFilename', '') == os.path.abspath(self.config.LOG_FILE)
        ...
        self.logger.info(f"✅ AI System loaded. Available models: {list(self.ai_system.models.keys())}")
        ...
        self.ev = EventLogger(path="data/events.jsonl", run_id=self.run_id, symbol=self.config.SYMBOL)
        # --- END PATCH ---
        ...
        self.swing_lookback = int(self.config.config_data.get("advanced", {}).get("swing_lookback", 12))
        self.max_spread_atr_frac = float(self.config.config_data.get("advanced", {}).get("dynamic_spread_atr_frac", 0.10))  # سخت‌گیری بیشتر
            ...
            if MT5_AVAILABLE and self.mt5_connected:
                ...
                if not sym:
                    ...
                    if not tick:
                        ...
                        spread_price = (tick.ask - tick.bid)
                        self.logger.info(f"Preflight: spread={spread_price:.5f} ask/bid=({tick.ask:.2f}/{tick.bid:.2f})")
                    self.logger.info(
    ...
    def _find_position_id_for_order(self, order_id: int, retries: int = 5, sleep_s: float = 0.5) -> Optional[int]:
        if not MT5_AVAILABLE:
            return None
    ...
    def _mfe_quantile_from_history(self, q: float = 0.6, lookback: int = 400) -> Optional[float]:
            ...
            if col and len(df) > 0:
                try:
                    ...
                    if len(s) >= 50:
                        ...
                        self.logger.info(f"MFE Q{q*100:.0f}% از {len(s)} معامله: {quantile_val:.2f}R")
                        return quantile_val
                except Exception as e:
    ...
    def _send_order(self, order_type, entry_price: float, sl_price: float, tp_price: float, volume: float, deviation: float) -> object:
    ...
    def _initialize_mt5_for_trading(self) -> bool:
        ...
        try:
            ...
            if self.config.LOGIN and self.config.PASSWORD and self.config.SERVER:
                if not mt5.login(login=self.config.LOGIN, password=self.config.PASSWORD, server=self.config.SERVER):
            ...
            self.logger.info(f"✅ MT5 connected | Account: {info.login} Balance: {info.balance:.2f} Equity: {info.equity:.2f}")
            self.logger.info(f"Symbol: {self.config.SYMBOL} MinLot: {sym.volume_min} MaxLot: {sym.volume_max} Step: {sym.volume_step}")
            return True
        except Exception as e:
    ...
    def _bootstrap_trailing(self):
        ...
        try:
            ...
            if open_pos:
                self.logger.info(f"🔗 Trailing bootstrap: registered {len(open_pos)} open positions.")
        except Exception as e:
    ...
    def _update_trailing(self):
        ...
        try:
            ...
            for ticket, p in open_pos.items():
                ...
                profit_points = (p.price_open - p.price_current) if p.type == mt5.POSITION_TYPE_SELL else (p.price_current - p.price_open)
                if profit_points > 0:
            ...
            for ticket, p, prof in candidates:
                if p.type == mt5.POSITION_TYPE_SELL:
                    ...
                    if p.sl is None or desired_sl < p.sl - 1e-6:
                        res = _modify_position_sltp(ticket, self.config.SYMBOL, new_sl=desired_sl, new_tp=p.tp, magic=self.config.MAGIC)
                        if res is None or res.retcode != mt5.TRADE_RETCODE_DONE:
                            self.logger.warning("⚠️ Trailing SLTP modify failed for pos=%s ret=%s", ticket, getattr(res, "retcode", None))
                            ...
                            self.logger.info("↘ Trailing move | pos=%s new_sl=%.2f", ticket, desired_sl)
                else:
                    # BUY
    ...
    def start(self):
        ...
        if self.mt5_connected:
            ...
            if acc:
                ...
                self.logger.info(f"Account: {acc.login} | Balance: {acc.balance:.2f} | Equity: {acc.equity:.2f}")
        self.logger.info("🚀 MR BEN Live Trading System v4.0 Starting...")
        self.logger.info(f"RUN_ID={self.run_id}")
        self.logger.info(f"CONFIG: SYMBOL={self.config.SYMBOL}, TF={self.config.TIMEFRAME_MIN}, BARS={self.config.BARS}, MAGIC={self.config.MAGIC}")
        self.logger.info(f"RISK: base={self.config.BASE_RISK}, conf_thresh={self.risk_manager.base_confidence_threshold}, max_open={self.config.MAX_OPEN_TRADES}")
        self.logger.info(f"LIMITS: daily_loss={self.config.MAX_DAILY_LOSS}, max_trades={self.config.MAX_TRADES_PER_DAY}, sessions={self.config.SESSIONS}, risk_based_vol={self.config.USE_RISK_BASED_VOLUME}")

        # --- BEGIN PATCH [emit start event] ---
        self.ev.emit("bot_start",
    ...
    def _today_pl_and_trades(self) -> Tuple[float, int]:
        ...
        try:
            ...
            cnt = sum(1 for d in deals if d.symbol == self.config.SYMBOL and d.entry == mt5.DEAL_ENTRY_IN)
    ...
    def _calculate_dynamic_sl_tp(self, df: pd.DataFrame, entry: float, signal: int, confidence: float, symbol: str) -> Tuple[float, float, dict]:
        """
        ...
        try:
            ...
            sl_mult *= (1.0 - self.k_sl * (conf - self.conf_min) / (self.conf_max - self.conf_min + 1e-9))
            tp_mult *= (1.0 + self.k_tp * (conf - self.conf_min) / (self.conf_max - self.conf_min + 1e-9))

            # تعدیل بر اساس رژیم نوسان
            if vol_ratio > 1.5:
                sl_mult *= 1.10; tp_mult *= 1.10
            elif vol_ratio < 0.8:
                sl_mult *= 0.90; tp_mult *= 0.90
            ...
            meta = {
                "atr": atr_now, "vol_ratio": float(vol_ratio),
                "sl_mult": float(sl_mult), "tp_mult": float(tp_mult),
                "spread_price": float(spread_price), "spread_frac": float(spread_frac),
                "R": float(r), "swing_unreliable": swing_unreliable
            }
            ...
            if signal == 1:
                return entry - 0.5, entry + 1.0, {"atr": 0.5, "vol_ratio": 1.0, "sl_mult": 1.0, "tp_mult": 2.0, "spread_price": 0.0, "spread_frac": 0.0, "R": 2.0}
            else:
                return entry + 0.5, entry - 1.0, {"atr": 0.5, "vol_ratio": 1.0, "sl_mult": 1.0, "tp_mult": 2.0, "spread_price": 0.0, "spread_frac": 0.0, "R": 2.0}
    ...
    def _volume_for_trade(self, entry: float, sl: float) -> float:
        ...
        max_volume = float(self.config.config_data.get("trading", {}).get("max_risk_volume_cap", self.config.MAX_LOT))
    ...
    def _execute_trade(self, signal_data: Dict[str, Any], df: pd.DataFrame) -> bool:
        try:
            if signal_data['signal'] == 1:
                side = "BUY"; order_type = mt5.ORDER_TYPE_BUY
            elif signal_data['signal'] == -1:
                side = "SELL"; order_type = mt5.ORDER_TYPE_SELL
            else:
                return False
            ...
            if atr_now and MT5_AVAILABLE:
                ...
                if info and tick:
                    ...
                    self.logger.info(f"Spread check: {spread_points:.1f} pts ({spread_price:.4f}) vs ATR threshold: {atr_threshold:.4f}")
            
            if not ok_spread:
                # --- BEGIN PATCH [emit spread_block] ---
                self.ev.emit("blocked_by_spread", spread_price=float(spread_price),
                             atr_now=float(atr_now or 0), threshold=float(atr_threshold))
                # --- END PATCH ---
                self.logger.info(f"Skip trade due to high spread: {spread_price:.4f} > {atr_threshold:.4f} (ATR-based)")
                return False
            ...
            sl_price, tp_price, meta = self._calculate_dynamic_sl_tp(
                df, entry_price, signal_data['signal'], signal_data.get('confidence', 0.5), self.config.SYMBOL
            )

            # Enforce min distance (trade_stops_level / freeze) + rounding
            is_buy = (signal_data['signal'] == 1)
            sl_price, tp_price = enforce_min_distance_and_round(self.config.SYMBOL, entry_price, sl_price, tp_price, is_buy)
            ...
            if R_post < 1.2:
                ...
                sl_try, _ = enforce_min_distance_and_round(self.config.SYMBOL, entry_price, sl_candidate, tp_price, is_buy)
                ...
                if R_try >= 1.2:
                    ...
                    desired_tp = entry_price + np.sign(tp_price - entry_price) * 1.2 * abs(entry_price - sl_price)
                    _, tp_try = enforce_min_distance_and_round(self.config.SYMBOL, entry_price, sl_price, desired_tp, is_buy)
                    ...
                    if R_try2 >= 1.2:
                        ...
                        self.logger.info(f"Skip trade due to low R after fallback")
                        return False
            ...
            if not MT5_AVAILABLE or not self.mt5_connected:
                if use_split:
                    self.logger.info(f"[DEMO] {side} SPLIT: Vol1={volume_total*self.tp_policy['tp1_share']:.2f} TP1={self._r_to_price(entry_price, sl_price, signal_data['signal'], self.tp_policy['tp1_r']):.2f}, Vol2={volume_total*(1-self.tp_policy['tp1_share']):.2f} TP2={self._r_to_price(entry_price, sl_price, signal_data['signal'], self.tp_policy['tp2_r']):.2f}")
                else:
                    self.logger.info(f"[DEMO] {side} at {entry_price:.2f} SL={sl_price:.2f} TP={tp_price:.2f} Vol={volume_total}")
                return True
            ...
            if MT5_AVAILABLE:
                ...
                if info and tick:
                    ...
                    deviation_mult = float(self.config.config_data.get("advanced", {}).get("deviation_multiplier", 1.5))
                    deviation = max(10, int(spread_pts * deviation_mult))
            
            if use_split:
                # محاسبه TP1 و TP2
                tp1_price = self._r_to_price(entry_price, sl_price, signal_data['signal'], self.tp_policy["tp1_r"])
                tp2_price = self._r_to_price(entry_price, sl_price, signal_data['signal'], max(self.tp_policy["tp2_r"], self.min_R_after_round))
                
                # رُندینگ TP ها
                _, tp1_price = enforce_min_distance_and_round(self.config.SYMBOL, entry_price, sl_price, tp1_price, is_buy)
                _, tp2_price = enforce_min_distance_and_round(self.config.SYMBOL, entry_price, sl_price, tp2_price, is_buy)
            ...
            self.ev.emit("order_attempt", side=side, entry=float(entry_price),
                         sl=float(sl_price), tp=float(tp_price), volume=float(volume_total),
                         R_post=float(R_post), use_split=bool(use_split))
            # --- END PATCH ---
            ...
            for vol, tp, tag in orders_to_send:
                ...
                if res is None or res.retcode != mt5.TRADE_RETCODE_DONE:
                    ...
                    tick = mt5.symbol_info_tick(self.config.SYMBOL)
                    if tick:
                        req_order["price"] = tick.ask if order_type == mt5.ORDER_TYPE_BUY else tick.bid
                    ...
                    # فیلینگ جایگزین
                    req_order["type_filling"] = getattr(mt5, "ORDER_FILLING_RETURN", req_order["type_filling"])
                    res = mt5.order_send(req_order)
                    order_fallback = True
                    
                    # اگر باز هم fail بود، FOK را تست کن
                    if res is None or res.retcode != mt5.TRADE_RETCODE_DONE:
                        req_order["type_filling"] = getattr(mt5, "ORDER_FILLING_FOK", req_order["type_filling"])
                        res = mt5.order_send(req_order)
            ...
            for res_data, req_order, tag, order_fallback in results:
                ...
                self.ev.emit("order_result", tag=tag,
                ...
                row = {
                    'run_id': self.run_id, 'timestamp': ts, 'symbol': req_order['symbol'], 'action': f"{side}_{tag}",
                    'entry_price': req_order['price'], 'sl_price': req_order['sl'], 'tp_price': req_order['tp'],
                    'volume': req_order['volume'], 'confidence': signal_data.get('confidence', 0.0),
                    'source': signal_data.get('source', 'Unknown'),
                    'atr': meta.get('atr', None), 'vol_ratio': meta.get('vol_ratio', None),
                    'sl_mult': meta.get('sl_mult', None), 'tp_mult': meta.get('tp_mult', None),
                    'spread_price': meta.get('spread_price', None), 'spread_frac': meta.get('spread_frac', None),
                ...
                if res_data is None:
                    ...
                    row.update({'mt5_order_id': 0, 'mt5_retcode': -1, 'mt5_executed': False, 'mt5_error': 'order_send None'})
                elif res_data.retcode == mt5.TRADE_RETCODE_DONE:
                    self.logger.info(f"✅ EXECUTED {tag} #{res_data.order}")
                    row.update({'mt5_order_id': res_data.order, 'mt5_retcode': res_data.retcode, 'mt5_executed': True})
                    
                    # نگهداری position برای trailing
                    executed_positions.append((res_data.order, req_order))
                        
                else:
                    self.logger.error(f"❌ Order {tag} failed: retcode={res_data.retcode} comment={res_data.comment}")
                    err_text = str(res_data.comment).replace("\n"," ").replace("\r"," ")
                    row.update({'mt5_order_id': 0, 'mt5_retcode': res_data.retcode, 'mt5_executed': False, 'mt5_error': err_text})
            ...
            if any(r and getattr(r[0], 'retcode', None) == mt5.TRADE_RETCODE_DONE for r in results):
                ...
                for ticket, p in open_pos.items():
                    if ticket not in self.trailing_registry:
                        ...
                        added += 1
                if added:
                    self.logger.info("🔗 Trailing attached to %d newly-opened positions: %s", added, list(open_pos.keys()))
            
            # --- BEGIN PATCH [Success evaluation based on at least one execution] ---
            executed_any = any(r and getattr(r[0], 'retcode', None) == mt5.TRADE_RETCODE_DONE for r in results)
            if executed_any:
            ...
            try:
                ...
                if self.session_start_equity:
                    ...
                    if ai and ai.equity <= self.session_start_equity * 0.98:
                        ...
                        self.ev.emit("kill_switch", reason="session_equity_drop",
                                     session_start_equity=self.session_start_equity, equity=float(ai.equity))
                        # --- END PATCH ---
                ...
                if "24h" in self.config.SESSIONS:
                    ...
                    if cycle % SESSION_LOG_EVERY == 0:
                        self.logger.info(f"❌ Outside allowed sessions; skipping. Current: {sess_now}, Allowed: {self.config.SESSIONS}")
                    time.sleep(self.config.SLEEP_SECONDS)
                    continue
                else:
                    if cycle % SESSION_LOG_EVERY == 0:
                        self.logger.info(f"✅ Session {sess_now} is allowed - proceeding with trade analysis")
                ...
                # Bar-gate logic
                bar_ts = pd.to_datetime(df['time'].iloc[-1]).to_pydatetime().replace(second=0, microsecond=0)
                ...
                if not self.risk_manager.can_open_new_trade(
                    ...
                    self.logger.info(f"Risk blocked new trade. open={open_count}/{self.config.MAX_OPEN_TRADES}")
                    time.sleep(self.config.RETRY_DELAY)
                    continue
                ...
                self.logger.info(f"📊 Signal tracking: current={sig['signal']}, last={self.last_signal}, consecutive={self.consecutive_signals}")
                ...
                if emergency_bypass:
                    self.logger.warning("🚨 EMERGENCY BYPASS ENABLED - Conformal gate completely disabled!")
                    soft_enabled = False
                    ...
                    try:
                        ...
                        if not soft_gate:
                            ...
                            if not ok or (p_hat is not None and p_hat < min_p):
                                self.logger.info(f"Conformal hard block: ok={ok}, p={p_hat:.3f} < {min_p}")
                                time.sleep(self.config.RETRY_DELAY)
                                continue
                        else:
                            # حالت نرم: فقط سخت‌تر کردن شرایط
                            if p_hat is not None and p_hat < hard_floor:
                                self.logger.info(f"Conformal below hard_floor: p={p_hat:.3f} < {hard_floor}")
                                time.sleep(self.config.RETRY_DELAY)
                                continue
                ...
                if conformal_p is not None and conformal_status == "active":
                    ...
                    self.logger.info("  p=%.3f -> adj_thr: %.3f (base: %.3f) | req_consec: %d",
                                    conformal_p, adj_thr, base_thr, req_consec)

                    # High-confidence override
                    effective_req_consec = 1 if sig['confidence'] >= (adj_thr + override_margin) else req_consec
                    if effective_req_consec != req_consec:
                        self.logger.info("  High-confidence override: conf=%.3f >= %.3f -> req_consec -> 1",
                                        sig['confidence'], adj_thr + override_margin)
                ...
                # Fallback: If Conformal is not available, use base thresholds
                    ...
                    adj_thr = base_thr
                    req_consec = self.config.CONSECUTIVE_SIGNALS_REQUIRED
                    self.logger.info(f"Using fallback thresholds: adj_thr={adj_thr:.2f}, req_consec={req_consec}")

                should = (
                    sig['confidence'] >= adj_thr and
                    self.consecutive_signals >= req_consec and
                    sig['signal'] != 0 and
                    open_count < self.config.MAX_OPEN_TRADES
                )

                # --- Comprehensive Trade Execution Check ---
                self.logger.info("🔍 Trade execution check:")
                self.logger.info("   Signal: %s", sig['signal'])
                self.logger.info("   Confidence: %.3f (threshold: %.3f)", sig['confidence'], adj_thr)
                self.logger.info("   Consecutive signals: %d (required: %d)", self.consecutive_signals, req_consec)
                ...
                if not should:
                    ...
                    if self.consecutive_signals < req_consec:
                        self.logger.info("   - Consecutive %d < %d", self.consecutive_signals, req_consec)
                    if sig['signal'] == 0:
                        self.logger.info("   - Signal is 0")
                    if open_count >= self.config.MAX_OPEN_TRADES:
                        self.logger.info("   - Max trades reached: %d/%d", open_count, self.config.MAX_OPEN_TRADES)
                else:
                ...
                if should:
                    ...
                    if ok:
                        ...
                        self.logger.info(f"✅ Trade executed successfully. Cooldown {cooldown_sec}s")
                    else:
                ...
                if cycle % 10 == 0:  # Every 10 cycles
                    ...
                    self.logger.info(f"   Last signal: {sig['signal']} (confidence: {sig['confidence']:.3f})")
