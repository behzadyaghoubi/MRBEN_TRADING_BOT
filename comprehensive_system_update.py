...
class ComprehensiveSystemUpdater:
    ...
    def _retrain_lstm_model(self):
        ...
        try:
            ...
            self.logger.info("📊 Model saved: models/mrben_lstm_comprehensive_retrain.h5")
    ...
    def _enhance_ml_filter(self):
        ...
        try:
            ...
            trade_logs = []
            log_files = ['logs/live_trades.csv', 'data/trade_log_clean.csv', 'data/trade_log.csv']
            ...
            market_data_paths = ['data/XAUUSD_PRO_M5_live.csv', 'data/XAUUSD_PRO_M5_enhanced.csv']
            ...
            y = (
                enhanced_data['signal']
                if 'signal' in enhanced_data.columns
                else enhanced_data['action']
            )
            ...
            self.logger.info("📊 Enhanced model saved: models/mrben_ml_filter_enhanced.joblib")
    ...
    def _diagnose_signal_pipeline(self):
        ...
        try:
            ...
            self.logger.info("✅ Signal pipeline diagnostics completed")
...
"""
            ...
            # Save report
            report_path = (
                f'logs/comprehensive_update_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
            )
            ...
            results_path = (
                f'logs/comprehensive_update_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
            )
    ...
    def _prepare_enhanced_ml_data(
        self, trade_logs: pd.DataFrame, market_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            for _, trade in trade_logs.iterrows():
                ...
                market_row = (
                    market_data[market_data['time'] <= trade_time].iloc[-1]
                    if len(market_data[market_data['time'] <= trade_time]) > 0
                    else None
                )
                ...
                if market_row is not None:
                    features = {
                        ...
                        'bb_position': (
                            market_row['close'] - market_row.get('bb_lower', market_row['close'])
                        )
                        / (
                            market_row.get('bb_upper', market_row['close'])
                            - market_row.get('bb_lower', market_row['close'])
                        ),
        ...
        try:
            ...
            for i, scenario in enumerate(test_scenarios):
                try:
                    ...
                    signal = self._generate_signal_for_test(
                        scenario, lstm_model, lstm_scaler, ml_filter
                    )
    ...
    def _generate_signal_for_test(self, scenario: dict, lstm_model, lstm_scaler, ml_filter) -> dict:
        ...
        try:
            ...
            if hasattr(lstm_model, 'predict_proba'):
                ...
                lstm_signal = (
                    1 if np.argmax(lstm_pred) == 2 else (-1 if np.argmax(lstm_pred) == 0 else 0)
                )
            else:
                # LSTM model
            ...
            ml_features = [
                ...
                scenario['macd'] - scenario['macd_signal'],
            ]
    ...
    def _apply_signal_pipeline_fixes(self, test_results: dict) -> dict:
        ...
        fixes = {'total_fixes': 0, 'fixes_applied': []}
        ...
        try:
            ...
            if total_signals > 0:
                ...
                if sell_percentage > 0.7:
                    fixes['fixes_applied'].append(
                        'Reduced SELL bias by adjusting signal thresholds'
                    )
                    fixes['total_fixes'] += 1
    ...
    def _execute_backtest(
        self, test_data: pd.DataFrame, lstm_model_path: str, ml_filter_path: str
    ) -> dict:
            ...
            for i in range(50, len(test_data)):  # Start from 50 to have enough history
                try:
                    ...
                    signal = self._generate_signal_for_backtest(
                        window, lstm_model, lstm_scaler, ml_filter
                    )
    ...
    def _generate_signal_for_backtest(
        self, window: pd.DataFrame, lstm_model, lstm_scaler, ml_filter
    ) -> dict:
        ...
        try:
            ...
            ml_features = [
                ...
                current['low'],
                current['close'],
                current.get('sma_20', current['close']),
                current.get('sma_50', current['close']),
                current.get('rsi', 50),
                current.get('macd', 0),
            ...
            return {'signal': final_signal, 'confidence': combined_confidence}
