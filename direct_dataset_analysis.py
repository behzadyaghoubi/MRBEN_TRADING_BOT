...
def direct_dataset_analysis():
    ...
    try:
        ...
        if buy_count > 0 and sell_count > 0:
            print("   ✅ دیتاست آماده برای بازآموزی LSTM است")
...
def create_next_steps_guide():
    ...
    guide_content = '''# 🚀 مراحل بعدی - MR BEN Trading System

## ✅ وضعیت فعلی:
- دیتاست مصنوعی متعادل ایجاد شد
- توزیع BUY/SELL متعادل است
- آماده برای بازآموزی LSTM

## 📋 مراحل بعدی:

### 1. رفع مشکل کیبورد (اولویت)
```cmd
# راهنمای دستی:
MANUAL_KEYBOARD_FIX.md
```
...
## 🎯 هدف نهایی:
- سیستم متعادل BUY/SELL
- عملکرد بهتر در بازار
- کاهش bias در سیگنال‌ها

## ⚠️ نکات مهم:
- حتماً مشکل کیبورد را حل کنید
...
'''
    ...
    try:
        ...
        print("   ✅ راهنمای مراحل بعدی ایجاد شد: NEXT_STEPS.md")
    except Exception as e:
