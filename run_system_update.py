...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                f'logs/execution_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8'
            ),
...
def run_command_safely(command, description=""):
    ...
    try:
        ...
        result = subprocess.run(
            clean_cmd, shell=True, capture_output=True, text=True, encoding='utf-8'
        )
...
def main():
    ...
    success = run_command_safely("python test_system_update.py", "System component test")
    ...
    success = run_command_safely(
        "python fixed_comprehensive_update.py", "Comprehensive system update"
    )
