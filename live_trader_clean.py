...
Examples:
  ...
  %(prog)s diagnose --symbol XAUUSD.PRO
    ...
    p_diag.add_argument("--symbol", default="XAUUSD.PRO", 
                       help="Symbol to diagnose (default: XAUUSD.PRO)")
...
class MT5LiveTrader:
        ...
        if MT5_AVAILABLE:
            try:
                ...
                if isinstance(symbol_diag, dict):
                    self.logger.info(f"🔍 Initial symbol diagnostics: filling_mode={symbol_diag.get('filling_mode')}, "
        ...
        try:
            ...
            if MT5_AVAILABLE:
                try:
                    symbol_diag = diagnose_symbol_parameters(self.config.SYMBOL)
                    if isinstance(symbol_diag, dict):
                        self.logger.info(f"🔍 Symbol diagnostics: filling_mode={symbol_diag.get('filling_mode')}, "
                                       f"stops_level={symbol_diag.get('stops_level')}, "
                                       f"lot_step={symbol_diag.get('lot_step')}, "
                                       f"point={symbol_diag.get('point')}, "
                                       f"digits={symbol_diag.get('digits')}")
    ...
    def _order_send_adaptive(self, request: dict):
        ...
        for m in candidates:
            ...
            if chk and chk.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                self.logger.info(f"⚠️ order_check failed for filling mode {m}: retcode={chk.retcode if chk else 'None'}")
    ...
    def _update_trailing_stops(self):
        ...
        try:
            now = datetime.now()
            # run every 15s max
            if (now - getattr(self, "last_trailing_update", now - timedelta(seconds=999))).seconds < 15:
                return
            ...
            step_points = getattr(self, "trailing_step_points", 50)  # 50 points = 0.50 if point=0.01
