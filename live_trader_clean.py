...
# Type hints for better Pyright support
from typing import TYPE_CHECKING

# Define types for Pyright without importing
if TYPE_CHECKING:
...
# Import modular components (conditional)
TRADING_SYSTEM_AVAILABLE = False
TRADING_LOOP_AVAILABLE = False

# Global variables to hold the classes
_GLOBAL_TRADING_SYSTEM_CLASS = None
_GLOBAL_TRADING_LOOP_MANAGER_CLASS = None


def _ensure_modular_components_loaded():
    """Ensure modular components are loaded when needed"""
    global TRADING_SYSTEM_AVAILABLE, TRADING_LOOP_AVAILABLE, _GLOBAL_TRADING_SYSTEM_CLASS, _GLOBAL_TRADING_LOOP_MANAGER_CLASS

    # Only attempt to load if not already successfully loaded
    if not TRADING_SYSTEM_AVAILABLE and not TRADING_LOOP_AVAILABLE:
        try:
            ...
            try:
                ...
                _GLOBAL_TRADING_SYSTEM_CLASS = TradingSystem
            ...
            try:
                ...
                _GLOBAL_TRADING_LOOP_MANAGER_CLASS = TradingLoopManager
...
if 'EventLogger' not in globals():
    ...
    class EventLogger:
        ...
        def log_trade_attempt(self, *a, **k):
            pass
...
if 'PerformanceMetrics' not in globals():
    ...
    class PerformanceMetrics:
        ...
        def record_memory_usage(self, *a, **k):
            pass
...
if 'MemoryMonitor' not in globals():
    ...
    class MemoryMonitor:
        ...
        def check_memory(self, *a, **k):
            return None

        def cleanup_memory(self):
            return 0

        def should_cleanup(self, *a, **k):
            return False
...
Examples:
  ...
  %(prog)s diagnose --symbol XAUUSD.PRO
        """,
    )
    ...
    p_live.add_argument(
        "--mode", choices=["live", "paper"], default="live", help="Trading mode (default: live)"
    )
    p_live.add_argument(
        "--symbol", default="XAUUSD.PRO", help="Trading symbol (default: XAUUSD.PRO)"
    )
    p_live.add_argument(
        "--config", default="config.json", help="Configuration file (default: config.json)"
    )
    p_live.add_argument(
        "--core",
    ...
    p_live.add_argument("--agent", action="store_true", help="Enable GPT-5 agent supervision")
    p_live.add_argument(
        "--agent-mode",
        choices=["observe", "guard", "auto"],
        default="guard",
        help="Agent mode: observe (read-only), guard (block risky), auto (auto-remediate)",
    )
    p_live.add_argument(
        "--regime", action="store_true", help="Enable regime detection & adaptive thresholds"
    )
    p_live.add_argument("--profile", action="store_true", help="Enable performance profiling")
    ...
    p_paper.add_argument("--agent", action="store_true", help="Enable GPT-5 agent supervision")
    p_paper.add_argument(
        "--agent-mode",
        choices=["observe", "guard", "auto"],
        default="guard",
    ...
    p_paper.add_argument("--profile", action="store_true", help="Enable performance profiling")

    # Backtest
    p_bt = sub.add_parser("backtest", help="Run backtest")
    p_bt.add_argument("--symbol", default="XAUUSD.PRO", help="Trading symbol (default: XAUUSD.PRO)")
    p_bt.add_argument("--from", dest="dt_from", help="Start date (YYYY-MM-DD)")
    p_bt.add_argument("--to", dest="dt_to", help="End date (YYYY-MM-DD)")
    p_bt.add_argument(
        "--config", default="config.json", help="Configuration file (default: config.json)"
    )
    p_bt.add_argument(
    ...
    p_bt.add_argument("--agent", action="store_true", help="Enable GPT-5 agent supervision")
    p_bt.add_argument(
        "--agent-mode",
        choices=["observe", "guard", "auto"],
        default="guard",
    ...
    p_sm.add_argument(
        "--minutes", type=int, default=5, help="Test duration in minutes (default: 5)"
    )
    p_sm.add_argument("--symbol", default="XAUUSD.PRO", help="Trading symbol (default: XAUUSD.PRO)")
    p_sm.add_argument(
        "--config", default="config.json", help="Configuration file (default: config.json)"
    )
    p_sm.add_argument(
    ...
    p_sm.add_argument("--agent", action="store_true", help="Enable GPT-5 agent supervision")
    p_sm.add_argument(
        "--agent-mode",
        choices=["observe", "guard", "auto"],
        default="guard",
    ...
    # Diagnostic command
    p_diag = sub.add_parser("diagnose", help="Run comprehensive symbol diagnostics")
    p_diag.add_argument(
        "--symbol", default="XAUUSD.PRO", help="Symbol to diagnose (default: XAUUSD.PRO)"
    )
    p_ag.add_argument(
        "--agent-mode",
        choices=["observe", "guard", "auto"],
    ...
    p_ag.add_argument("--symbol", default="XAUUSD.PRO", help="Trading symbol (default: XAUUSD.PRO)")
    p_ag.add_argument(
        "--config", default="config.json", help="Configuration file (default: config.json)"
    )
    p_ag.add_argument(
    ...
    p_ag.add_argument("--profile", action="store_true", help="Enable performance profiling")
...
def apply_regime_and_threshold(core_ctx, symbol: str, cfg, raw_conf: float):
    ...
    try:
        ...
        try:
            bars = (
                core_ctx.get_recent_bars(symbol) if hasattr(core_ctx, 'get_recent_bars') else None
            )
            ...
            logger.info(
                f"🔍 Advanced regime detected: {regime.label.value} (confidence: {regime.confidence:.3f})"
            )
            ...
            if SCORER_AVAILABLE:
                ...
                decision = {'adj_conf': raw_conf, 'threshold': 0.5, 'allow_trade': raw_conf >= 0.5}
        ...
        return None, {'adj_conf': raw_conf, 'threshold': 0.5, 'allow_trade': raw_conf >= 0.5}
...
def _fallback_regime_detection(core_ctx, symbol: str, raw_conf: float, cfg=None):
    ...
    try:
        ...
        if bars is not None and len(bars) > 20:
            ...
            }, {'adj_conf': adj_conf, 'threshold': threshold, 'allow_trade': allow_trade}
        else:
            ...
            }, {'adj_conf': raw_conf, 'threshold': 0.5, 'allow_trade': raw_conf >= 0.5}
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if core_type == "legacy":
                ...
                # Use modular system - ensure components are loaded
                _ensure_modular_components_loaded()
                if TRADING_SYSTEM_AVAILABLE:
                    core = _GLOBAL_TRADING_SYSTEM_CLASS(cfg)  # Use the global class
                    logger.info("✅ Trading system initialized (modular mode)")
                else:
                    logger.warning("⚠️ Modular system not available, falling back to legacy")
                    core = MT5LiveTrader(cfg)
                    logger.info("✅ Trading system initialized (legacy mode - fallback)")
            else:
...
def cmd_backtest(args):
    ...
    try:
        ...
        try:
            ...
            result = backtest.run(
                symbol=args.symbol, dt_from=args.dt_from, dt_to=args.dt_to, cfg=cfg
            )
...
def main():
    ...
    try:
        ...
        if args.cmd == "live":
            ...
            return run_symbol_diagnostics(args.symbol)
        else:
...
# TensorFlow/Keras imports with Pyright-compliant handling
TENSORFLOW_AVAILABLE = False
load_model: LoadModelType = None


def _try_import_tensorflow() -> tuple[bool, LoadModelType]:
    """Try to import TensorFlow/Keras and return availability and load_model function"""
    try:
        # Try TensorFlow 2.x import first
        import tensorflow as tf

        # Use getattr to avoid direct import that Pyright sees
        ...
        if keras_module:
            ...
            if models_module:
                ...
                if load_model_func:
                    print("TensorFlow 2.x with Keras loaded successfully")
    ...
    try:
        ...
        models_module = getattr(keras, 'models', None)
        if models_module:
            load_model_func = getattr(models_module, 'load_model', None)
            if load_model_func:
                print("Standalone Keras loaded successfully")
                return True, load_model_func
    except Exception as e:
        print(f"⚠️ Standalone Keras import failed: {e}")

    return False, None


# Initialize TensorFlow availability (will be set when needed)
TENSORFLOW_AVAILABLE = None
load_model = None


def _ensure_tensorflow_loaded():
    """Ensure TensorFlow is loaded when needed"""
    global TENSORFLOW_AVAILABLE, load_model
    if TENSORFLOW_AVAILABLE is None:
        TENSORFLOW_AVAILABLE, load_model = _try_import_tensorflow()


# Fallback function when TensorFlow is not available
if not TENSORFLOW_AVAILABLE:

    def load_model_fallback(*args, **kwargs) -> Any:
        """Fallback function when TensorFlow/Keras is not available"""
        raise ImportError(
            "TensorFlow/Keras not available. Please install with: pip install tensorflow"
        )

    load_model = load_model_fallback
    print("Using fallback load_model function - AI features will be limited")
...
def _check_ai_availability():
    """Check AI availability when needed"""
    _ensure_tensorflow_loaded()
    return TENSORFLOW_AVAILABLE and SKLEARN_AVAILABLE and JOBLIB_AVAILABLE
...
def _ensure_ai_loaded():
    """Ensure AI stack is loaded when needed"""
    global AI_AVAILABLE
    if AI_AVAILABLE is None:
        AI_AVAILABLE = _check_ai_availability()
    if not AI_AVAILABLE:
        print("AI stack not available - some features will be limited")
        print("   To enable full AI features, install: pip install tensorflow scikit-learn joblib")
    else:
        print("Full AI stack available")
...
def _map_symbol_to_order_filling(sym_fm: int):
    ...
    if sym_fm == mt5.SYMBOL_FILLING_FOK:
        return mt5.ORDER_FILLING_FOK
    if sym_fm == mt5.SYMBOL_FILLING_IOC:
        return mt5.ORDER_FILLING_IOC
    if sym_fm == mt5.SYMBOL_FILLING_RETURN:
        return mt5.ORDER_FILLING_RETURN
    return None


def diagnose_symbol_parameters(symbol: str):
    """Diagnose symbol's filling mode, trade mode, and other parameters"""
    try:
        if not MT5_AVAILABLE:
        ...
        # Get key parameters
        filling_mode = getattr(si, 'filling_mode', 'Unknown')
        trade_mode = getattr(si, 'trade_mode', 'Unknown')
        stops_level = getattr(si, 'trade_stops_level', 'Unknown')
        lot_step = getattr(si, 'volume_step', 'Unknown')
        point = getattr(si, 'point', 'Unknown')
        digits = getattr(si, 'digits', 'Unknown')

        # Get all available fields for debugging
        all_fields = si._asdict()
        ...
        return f"Error diagnosing symbol {symbol}: {e}"
...
class MT5Config:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.INBAR_MIN_STRUCT_BUFFER_ATR = float(
                advanced.get("inbar_min_struct_buffer_atr", 0.8)
            )
            ...
            self.SPREAD_HYSTERESIS_FACTOR = float(
                execution_cfg.get("spread_hysteresis_factor", 1.05)
            )
            ...
            self.CONFORMAL_TREAT_ZERO_AS_BLOCK = bool(
                conformal_cfg.get("treat_zero_as_block", True)
            )
            self.CONFORMAL_MAX_CONF_BUMP_FLOOR = float(
                conformal_cfg.get("max_conf_bump_floor", 0.05)
            )
    ...
    def check_memory(self, force: bool = False) -> float | None:
        ...
        try:
            ...
            try:
                ...
                # Log memory usage
                self.logger.info(
                    f"💾 Memory check: {memory_mb:.1f} MB (collected {collected} objects)"
                )
...
class MT5LiveTrader:
    ...
    def __init__(self, config: MT5Config):
        ...
        # Diagnostic: Log initial symbol parameters
        if MT5_AVAILABLE:
            try:
                symbol_diag = diagnose_symbol_parameters(self.config.SYMBOL)
                if isinstance(symbol_diag, dict):
                    self.logger.info(
                        f"🔍 Initial symbol diagnostics: filling_mode={symbol_diag.get('filling_mode')}, "
                        f"stops_level={symbol_diag.get('stops_level')}, "
                        f"lot_step={symbol_diag.get('lot_step')}, "
                        f"point={symbol_diag.get('point')}, "
                        f"digits={symbol_diag.get('digits')}"
                    )
                else:
                    self.logger.info(f"🔍 Initial symbol diagnostics: {symbol_diag}")
            except Exception as e:
                self.logger.debug(f"Initial symbol diagnostics failed: {e}")
    ...
    def _initialize_agent_components(self):
        ...
        try:
            ...
            self.risk_gate = AdvancedRiskGate(config_data)

            # Initialize playbooks
            self.playbooks = AdvancedPlaybooks(config_data)

            # Initialize ML integration
            self.ml_integration = MLIntegration(config_data)

            # Initialize predictive maintenance
            self.predictive_maintenance = PredictiveMaintenance(config_data)

            # Initialize advanced alerting
            self.advanced_alerting = AdvancedAlerting(config_data)
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            # Initialize agent components if available
            try:
                self._initialize_agent_components()
            except Exception as e:
                self.logger.warning(f"⚠️ Agent components initialization failed: {e}")
    ...
    def _init_mt5(self):
        ...
        try:
            ...
            if mt5.symbol_select(symbol, True):
                ...
                self.logger.warning(
                    f"⚠️ Primary symbol {symbol} not available, trying alternatives..."
                )
                ...
                for alt_symbol in alternatives:
                    if alt_symbol != symbol and mt5.symbol_select(alt_symbol, True):
                        self.logger.warning(
                            f"⚠️ Primary symbol {symbol} unavailable, using {alt_symbol} instead"
                        )
                ...
                if not symbol_selected:
                    self.logger.error(
                        f"❌ No suitable symbol found. Tried: {[symbol] + alternatives}"
                    )
                    self.logger.error(
                        f"Available symbols: {[s.name for s in mt5.symbols_get()[:10]] if mt5.symbols_get() else 'None'}"
                    )
    ...
    def _preflight_production(self):
        ...
        try:
            ...
            if self.config.DEMO_MODE:
                self.logger.warning(
                    "⚠️ DEMO_MODE=True; live requested. Set DEMO_MODE=False in config for real trades."
                )

            # 1) Credentials (if DEMO_MODE False)
            if not self.config.DEMO_MODE and not (
                self.config.LOGIN and self.config.PASSWORD and self.config.SERVER
            ):
    ...
    def start(self):
        ...
        try:
            ...
            if not PANDAS_AVAILABLE:
                self.logger.error(
                    "pandas is required for legacy trading loop. Please install pandas."
                )
                return False
        ...
        while self.running:
            ...
            try:
                ...
                if (
                    ...
                    and self.agent
                    and getattr(self.agent, "mode", "guard") != "observe"
                ):
                ...
                if self.predictive_maintenance and self.metrics.cycle_count % 60 == 0:
                    try:
                        ...
                        if (
                            err_rate
                            > (self.config.config_data.get("agent", {}).get("error_rate_halt", 5))
                            / 60.0
                        ):
                            if self.agent:
                ...
                if df is None or len(df) < 50:
                    self.logger.warning(
                        f"Insufficient data: {len(df) if df is not None else 'None'} bars; retrying..."
                    )
                ...
                self.logger.debug(
                    f"📊 Data quality: {len(df)} bars, last close: {df['close'].iloc[-1]:.5f}, last time: {df['time'].iloc[-1]}"
                )
                ...
                if signal['signal'] == 0:
                    ...
                    if signal['signal'] == self.last_signal:
                        ...
                        self.logger.info(
                            f"🔄 Consecutive signal {self.consecutive_signals} for {signal['signal']} (need {self.config.CONSECUTIVE_SIGNALS_REQUIRED})"
                        )
                    else:
                        ...
                        self.logger.info(
                            f"🆕 New signal type: {signal['signal']}, reset counter to 1"
                        )

                # Log signal details for debugging
                self.logger.info(
                    f"📊 Signal: {signal['signal']} (confidence: {signal['confidence']:.3f}, consecutive: {self.consecutive_signals}/{self.config.CONSECUTIVE_SIGNALS_REQUIRED})"
                )
                ...
                if spread_pts is not None and spread_pts > self.max_spread_points:
                    ...
                    self.logger.info(
                        f"⏸️ Blocked by spread gate: {spread_pts:.0f} > {self.max_spread_points}"
                    )
                ...
                if (
                    hasattr(self, 'config')
                    and hasattr(self.config, 'CONFORMAL_ENABLED')
                    and self.config.CONFORMAL_ENABLED
                ):
                    try:
                ...
                try:
                    if hasattr(self, 'config') and hasattr(self.config, 'regime_config'):
                        regime_result = apply_regime_and_threshold(
                            self, self.config.SYMBOL, self.config, signal['confidence']
                        )
                        if regime_result and len(regime_result) > 0:
                            ...
                            decision_card.adj_conf = regime_data.get(
                            ...
                            self.logger.info(
                                f"🔍 Regime detected: {regime_data.get('label', 'UNKNOWN')} (adj_conf: {regime_data.get('adj_conf', 0):.3f})"
                            )
                        else:
                            self.logger.warning(
                                "Regime detection returned no results, using defaults"
                            )
                ...
                self.logger.info(
                    f"   Price: {df['close'].iloc[-1]:.5f} | SMA20: {df['sma_20'].iloc[-1]:.5f} | SMA50: {df['sma_50'].iloc[-1]:.5f}"
                )
                self.logger.info(
                    f"   Regime: {decision_card.regime_label} | Adj Conf: {decision_card.adj_conf:.3f}"
                )
                self.logger.info(
                    f"   Threshold: {decision_card.threshold:.3f} | Allow Trade: {decision_card.allow_trade}"
                )
                self.logger.info(
                    f"   Spread: {decision_card.spread_pts:.1f} pts | Open Positions: {decision_card.open_positions}"
                )
                    ...
                    try:
                        ...
                        if decision_card.spread_pts is not None:
                            if decision_card.spread_pts > self.max_spread_points:
                                decision_card.allow_trade = False
                                self.logger.info(
                                    f"⏸️ Blocked by spread gate: {decision_card.spread_pts:.0f} > {self.max_spread_points}"
                                )
                ...
                self.logger.info(
                    f"🔍 Trade execution check: {'✅ APPROVED' if should_execute else '❌ BLOCKED'}"
                )
                ...
                if should_execute:
                    ...
                    if signal['signal'] == 0:
                        ...
                        self.logger.info(
                            f"   ❌ Confidence {signal['confidence']:.3f} below threshold 0.5"
                        )
                    elif self.consecutive_signals < self.config.CONSECUTIVE_SIGNALS_REQUIRED:
                        self.logger.info(
                            f"   ❌ Consecutive signals {self.consecutive_signals} below required {self.config.CONSECUTIVE_SIGNALS_REQUIRED}"
                        )
                    else:
    ...
    def _get_market_data(self):
        ...
        try:
            ...
            # Get bars data
            bars = mt5.copy_rates_from_pos(symbol, self._get_timeframe_enum(), 0, self.config.BARS)
            ...
            self.logger.debug(
                f"📊 Market data: {len(df)} bars, last timestamp: {df['time'].iloc[-1]}"
            )

            # Check data quality
            if len(df) < 50:
                self.logger.warning(
                    f"Insufficient data: {len(df)} bars, need at least 50 for SMA calculation"
                )
                return None
            ...
            if not hasattr(self, '_symbol_info_logged'):
                try:
                    ...
                    if symbol_info:
                        self.logger.info(
                            f"📈 Symbol info: {symbol} - Digits: {symbol_info.digits}, Point: {symbol_info.point}, Trade mode: {symbol_info.trade_mode}"
                        )
    ...
    def _generate_signal(self, df):
        ...
        try:
            ...
            if len(df) < 50:
                self.logger.warning(
                    f"Insufficient data for SMA calculation: {len(df)} bars, need at least 50"
                )
                return None
            ...
            self.logger.debug(
                f"Signal generation - Price: {current_price:.5f}, SMA20: {sma_20:.5f}, SMA50: {sma_50:.5f}"
            )
            ...
            if self.ml_integration:
                try:
                    ensemble_result = self.ml_integration.generate_ensemble_signal(
                        base_signal=sma_signal, base_confidence=sma_confidence, df=df
                    )
                    if ensemble_result:
                        ...
                        self.logger.debug(
                            f"ML Ensemble signal: {final_signal} (conf: {final_confidence:.3f})"
                        )
                        return {
                            'signal': final_signal,
        ...
        try:
            ...
            if ai_confidence > 0:
                ...
                if sma_signal == ai_signal:
                    ...
                    final_confidence = (
                        sma_confidence * sma_weight + ai_confidence * ai_weight
                    ) * 1.2
            ...
            if final_confidence < 0.3:
                ...
                final_signal = (
                    sma_signal
                    if ai_confidence == 0
                    else (ai_signal if ai_confidence > sma_confidence else sma_signal)
                )
    ...
    def _should_execute_trade_with_conformal(self, signal, decision_card):
        ...
        try:
            ...
            if self.consecutive_signals < required_signals:
                self.logger.info(
                    f"🔒 Need {required_signals} consecutive signals, have {self.consecutive_signals}"
                )
                return False
            ...
            self.logger.info(
                f"✅ Trade execution approved: conf={adj_conf:.3f}, threshold={threshold:.3f}, consecutive={self.consecutive_signals}"
            )
            return True
    ...
    def _apply_conformal_gate(self, signal, decision_card):
        ...
        try:
            ...
            if (
                cfg.CONFORMAL_EMERGENCY_BYPASS
                and self.consecutive_signals >= cfg.CONFORMAL_EXTRA_CONSEC_FLOOR
            ):
                allow_trade = True
    ...
    def _check_daily_loss_limit(self):
        ...
        try:
            ...
            daily_pnl_pct = (
                daily_pnl / self._get_account_balance() if self._get_account_balance() > 0 else 0
            )
    ...
    def _execute_trade(self, signal, df):
        ...
        try:
            ...
            sl, tp = enforce_min_distance_and_round(
                self.config.SYMBOL, current_price, sl, tp, is_buy
            )

            # Calculate position size based on risk
            volume = self._calculate_position_size(current_price, sl)

            # Diagnostic: Log symbol parameters for debugging
            if MT5_AVAILABLE:
                try:
                    symbol_diag = diagnose_symbol_parameters(self.config.SYMBOL)
                    if isinstance(symbol_diag, dict):
                        self.logger.info(
                            f"🔍 Symbol diagnostics: filling_mode={symbol_diag.get('filling_mode')}, "
                            f"stops_level={symbol_diag.get('stops_level')}, "
                            f"lot_step={symbol_diag.get('lot_step')}, "
                            f"point={symbol_diag.get('point')}, "
                            f"digits={symbol_diag.get('digits')}"
                        ...
                        self.logger.info(f"🔍 Symbol diagnostics: {symbol_diag}")
                except Exception as e:
                    self.logger.debug(f"Symbol diagnostics failed: {e}")

            self.logger.info(
                f"Executing {side} signal: price={current_price:.5f}, SL={sl:.5f}, TP={tp:.5f}, ATR={atr:.5f}"
            )
            ...
            if MT5_AVAILABLE and not self.config.DEMO_MODE:
                ...
                if success:
                    ...
                    self.logger.error("❌ MT5 order placement failed")
                    return False
            else:
    ...
    def _execute_tp1_partial_close(self, ticket, registry, position):
        ...
        try:
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                self.logger.info(
                    f"✅ TP1 partial close executed: {close_volume} lots at {registry['tp1_price']:.5f}"
                )
    ...
    def _execute_tp2_close(self, ticket, registry, position):
        ...
        try:
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                self.logger.info(
                    f"✅ TP2 full close executed: {registry['volume']} lots at {registry['tp2_price']:.5f}"
                )
    ...
    def _set_breakeven_stop(self, ticket, registry, position):
        ...
        try:
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                self.logger.error(
                    f"Breakeven stop modification failed: {result.retcode} - {result.comment}"
                )
    ...
    def _order_send_adaptive(self, request: dict):
        """Send order with robust filling mode selection using order_check to avoid 10030 error"""
        # 1) First try order_check to find allowed filling modes
        ...
        # 2) Try order_check first to find allowed modes
        allowed_modes = []
        for m in candidates:
            req = dict(request)
            req["type_filling"] = m
            # Ensure proper types
            if isinstance(req.get("price"), (np.float64, np.float32)):
                req["price"] = float(req["price"])
            if isinstance(req.get("volume"), (np.float64, np.float32)):
                req["volume"] = float(req["volume"])
            req["type_time"] = mt5.ORDER_TIME_GTC

            # Use order_check to validate before sending
            chk = mt5.order_check(req)
            if chk and chk.retcode == mt5.TRADE_RETCODE_DONE:
                allowed_modes.append(m)
                self.logger.info(f"✅ order_check passed for filling mode: {m}")
            else:
                self.logger.info(
                    f"⚠️ order_check failed for filling mode {m}: retcode={chk.retcode if chk else 'None'}"
                )

        # 3) If we found allowed modes, try them first
        if allowed_modes:
            candidates = allowed_modes + [m for m in candidates if m not in allowed_modes]
            self.logger.info(f"🎯 Using order_check validated modes first: {allowed_modes}")

        # 4) Sequential attempt with logging
        last_result = None
        for m in candidates:
            req = dict(request)
            req["type_filling"] = m
            ...
            # If error is not 10030, continue but log it
            if res is not None and res.retcode != 10030:
                self.logger.warning(
                    f"order_send retcode={res.retcode} with filling={m}; trying next candidate..."
                )
    ...
    def _place_mt5_order(self, side, entry_price, sl, tp, volume):
        ...
        try:
            ...
            if result is None:
                self.logger.error("❌ MT5 order placement failed: order_send returned None")
                return False
            ...
            if result.order:
                self._register_position_for_trailing(
                    result.order, side, entry_price, sl, tp, volume
                )
    ...
    def _update_trailing_stops(self):
        ...
        try:
            ...
            # run every 15s max
            if (
                now - getattr(self, "last_trailing_update", now - timedelta(seconds=999))
            ).seconds < 15:
                return
            ...
            info = mt5.symbol_info(self.config.SYMBOL)
            if not info:
                self.logger.debug("Trailing: no symbol info; skip")
                self.last_trailing_update = now
                return

            tick = mt5.symbol_info_tick(self.config.SYMBOL)
            if not tick:
                self.logger.debug("Trailing: no tick; skip")
                self.last_trailing_update = now
                return

            point = info.point or 0.01

            # trailing params (conservative defaults)
            step_points = getattr(
                self, "trailing_step_points", 50
            )  # 50 points = 0.50 if point=0.01
            buffer_points = getattr(self, "trailing_buffer_points", 20)

            # allow override via config.advanced if present
            try:
                adv = getattr(self.config, "config_data", {}).get("advanced", {})
                step_points = int(adv.get("trailing_step_points", step_points))
                buffer_points = int(adv.get("trailing_buffer_points", buffer_points))
            except Exception:
                pass
            ...
            for pos in positions:
                try:
                    ...
                    curr_sl = float(pos.sl) if pos.sl not in (None, 0.0) else None

                    # current executable price (what affects PnL / SL side)
                    px = float(tick.bid if is_buy else tick.ask)
                    profit_points = (px - entry) / point if is_buy else (entry - px) / point

                    # trail only if in profit beyond step
                    if profit_points <= step_points:
                        continue

                    # propose new SL just behind price with a small buffer
                    if is_buy:
                        proposed_sl = px - buffer_points * point
                        # never lower SL for BUY (only move up)
                        if curr_sl is not None and proposed_sl <= curr_sl:
                            continue
                    else:
                        proposed_sl = px + buffer_points * point
                        # never higher SL for SELL (only move down)
                        if curr_sl is not None and proposed_sl >= curr_sl:
                            continue

                    # respect broker stops/freeze distance and rounding
                    safe_sl, _ = enforce_min_distance_and_round(
                        self.config.SYMBOL, entry=px, sl=proposed_sl, tp=0.0, is_buy=is_buy
                    )

                    # if still not an improvement, skip
                    if curr_sl is not None:
                        if is_buy and safe_sl <= curr_sl:
                            continue
                        if (not is_buy) and safe_sl >= curr_sl:
                            continue
                    ...
                    req = {
                        ...
                        "symbol": self.config.SYMBOL,
                        "sl": float(safe_sl),
                        "tp": float(pos.tp) if pos.tp not in (None, 0.0) else 0.0,
                    ...
                    if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                        self.logger.debug(
                            f"Trailing SL updated: ticket={pos.ticket} newSL={safe_sl:.2f} (profit_pts={profit_points:.1f})"
                        )
                    else:
                        self.logger.debug(
                            f"Trailing SL update rejected: ticket={pos.ticket} retcode={getattr(res,'retcode',None)}"
                        )
                except Exception as e:
                    self.logger.debug(f"Trailing per-position error: {e}")

            self.last_trailing_update = now

        except Exception as e:
            # never let trailing crash the main loop
            self.logger.error(f"Error updating trailing stops: {e}")
            self.last_trailing_update = datetime.now()
...
def run_symbol_diagnostics(symbol: str = "XAUUSD.PRO"):
    """Run comprehensive symbol diagnostics and print results"""
    ...
    try:
        ...
        print("\n📋 Key Parameters:")
        print(f"  Filling Mode: {getattr(si, 'filling_mode', 'Unknown')}")
        print(f"  Trade Mode: {getattr(si, 'trade_mode', 'Unknown')}")
        print(f"  Stops Level: {getattr(si, 'trade_stops_level', 'Unknown')} points")
        print(f"  Lot Step: {getattr(si, 'volume_step', 'Unknown')}")
        print(f"  Point: {getattr(si, 'point', 'Unknown')}")
        print(f"  Digits: {getattr(si, 'digits', 'Unknown')}")
        print(f"  Min Lot: {getattr(si, 'volume_min', 'Unknown')}")
        print(f"  Max Lot: {getattr(si, 'volume_max', 'Unknown')}")

        # Test order_check with different filling modes
        print("\n🧪 Testing order_check with different filling modes...")

        # Get current tick for realistic testing
        tick = mt5.symbol_info_tick(symbol)
        if tick:
            test_price = (tick.bid + tick.ask) / 2
            ...
            for mode_name, mode_value in [
                ...
                chk = mt5.order_check(test_req)
                if chk:
                    print(f"    {mode_name}: retcode={chk.retcode} - {chk.comment}")
                else:
                    print(f"    {mode_name}: order_check returned None")
        else:
            print("  ❌ No tick data available for testing")
