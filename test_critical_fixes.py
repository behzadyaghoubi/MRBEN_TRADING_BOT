...
from live_trader_clean import (
    is_spread_ok_dynamic, 
    MT5Config, 
    EnhancedRiskManager,
    MT5LiveTrader
)
...
class TestCriticalFixes(unittest.TestCase):
    ...
    def test_trailing_stop_position_ticket_fix(self, mock_mt5):
        ...
        for p in (mock_mt5.positions_get(symbol=trader.config.SYMBOL) or []):
            if (abs(p.volume) == mock_request['volume'] and
                abs(p.price_open - mock_request['price']) < (mock_symbol_info.point * 5)):
                pos = p
                break
        ...
        if pos:
            trader.risk_manager.add_trailing_stop(pos.ticket, mock_request['price'], mock_request['sl'], True)
