...
class LiveSystemMonitor:
    ...
    def _display_system_health(self):
        ...
        recent_trades = [
            t for t in self.trade_history if self._is_recent(t['timestamp'], minutes=30)
        ]
        recent_signals = [
            s for s in self.signal_history if self._is_recent(s['timestamp'], minutes=5)
        ]
    ...
    def generate_report(self, hours=24):
        ...
        recent_trades = [
            t for t in self.trade_history if self._parse_timestamp(t['timestamp']) >= cutoff_time
        ]
        recent_signals = [
            s for s in self.signal_history if self._parse_timestamp(s['timestamp']) >= cutoff_time
        ]
