#!/usr/bin/env python3
"""
Final Working MR BEN AI Live Trading System
Uses the quick fix system for reliable operation
"""
...
# Import the working quick fix system
from quick_fix_system import QuickFixMRBENSystem


class FinalLiveTrader:
    """
    Final Working Live Trading System with MR BEN AI
    """

    def __init__(self, config_path: str = "advanced_live_config.json"):
        self.config = self._load_config(config_path)
        self.logger = self._setup_logger()
        self.ai_system = QuickFixMRBENSystem()
        self.trade_history = []
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        default_config = {
            ...
            'session_filters': {'Asia': True, 'London': True, 'NY': True},
        }
    ...
    def _setup_logger(self):
        ...
        logger = logging.getLogger('FinalLiveTrader')
        ...
        # File handler
        log_file = f'logs/final_live_trader_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    def execute_trade(self, signal: dict[str, Any], market_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"🎯 Executing {trade_type} trade: Entry={entry_price:.2f}, SL={stop_loss:.2f}, TP={take_profit:.2f}"
            )
    ...
    def _close_position(self, reason: str, exit_price: float):
        ...
        try:
            ...
            self.logger.info(f"{profit_emoji} Position closed: {reason}, Profit: {profit:.2f}")
    ...
    def generate_performance_report(self):
        ...
        try:
            ...
            report = f"""
============================================================
FINAL MR BEN AI LIVE TRADING - PERFORMANCE REPORT
============================================================
...
============================================================
Report generated by Final MR BEN AI Live Trading System
============================================================
"""

            # Save report
            report_path = f'logs/final_performance_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    ...
    def run_live_trading(self, duration_minutes: int = 60):
        ...
        try:
            self.logger.info(
                f"🚀 Starting Final MR BEN AI Live Trading for {duration_minutes} minutes"
            )
            ...
            for data_path in data_paths:
                if os.path.exists(data_path):
                    if 'ml_filter' not in self.ai_system.models:
                        self.ai_system.train_simple_ml_filter(data_path)
                    break
            ...
            while datetime.now() < end_time:
                try:
                    ...
                    # Log current status
                    signal_emoji = (
                        "🟢" if signal['signal'] == 1 else "🔴" if signal['signal'] == -1 else "🟡"
                    )
                    position_emoji = "📈" if self.current_position else "⏸️"

                    self.logger.info(
                        f"Iteration {iteration}: {signal_emoji} Signal: {signal['signal']}, Confidence: {signal['confidence']:.3f}, {position_emoji} Position: {'OPEN' if self.current_position else 'NONE'}"
                    )
            ...
            self.logger.info("✅ Final MR BEN AI Live Trading completed successfully")
...
def main():
    """Main function to run final live trading"""
    print("🎯 Final MR BEN AI Live Trading System")
    print("=" * 60)

    # Create and run live trader
    trader = FinalLiveTrader()

    # Run for 30 minutes (demo)
    trader.run_live_trading(duration_minutes=30)

    print("✅ Final MR BEN AI Live Trading completed!")
