...
"""
    ...
    def run_live_trading(self, duration_minutes: int = 60):
        ...
        try:
            self.logger.info(f"🚀 Starting Final MR BEN AI Live Trading for {duration_minutes} minutes")
            ...
            while datetime.now() < end_time:
                try:
                    ...
                    self.logger.info(f"Iteration {iteration}: {signal_emoji} Signal: {signal['signal']}, Confidence: {signal['confidence']:.3f}, {position_emoji} Position: {'OPEN' if self.current_position else 'NONE'}")
