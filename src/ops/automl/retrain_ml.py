...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('logs/automl_ml.log'), logging.StreamHandler()],
)
...
class MLRetrainer:
    ...
    ) -> tuple[Any, StandardScaler, float, float]:
    ...
    def _evaluate_model(
        self, model: Any, scaler: StandardScaler, X: np.ndarray, y: np.ndarray
    ) -> dict[str, float]:
    ...
    def _should_promote(self, new_metrics: dict[str, float]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Promotion decision: AUC improvement {auc_improvement:.3f}, F1 improvement {f1_improvement:.3f}"
            )
