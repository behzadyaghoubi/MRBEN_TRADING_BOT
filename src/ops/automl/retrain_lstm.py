...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/automl_lstm.log'),
...
class LSTMRetrainer:
    ...
    def _prepare_sequence_dataset(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            logger.info(f"Sequence dataset prepared: {X.shape[0]} samples, {X.shape[1]} timesteps, {X.shape[2]} features")
            return X, y
    ...
    def _evaluate_lstm(self, model: tf.keras.Model, X: np.ndarray, y: np.ndarray) -> Dict[str, float]:
    ...
    def _should_promote(self, new_metrics: Dict[str, float]) -> bool:
        ...
        try:
            ...
            logger.info(f"LSTM promotion decision: AUC improvement {auc_improvement:.3f}, F1 improvement {f1_improvement:.3f}")
