...
from .schemas import DecisionOutcome
...
class DecisionStore:
    ...
    def __init__(
        ...
        self.storage_stats = {"jsonl_writes": 0, "parquet_writes": 0, "memory_cleanups": 0}
    ...
    def get_decision(self, decision_id: str) -> DecisionOutcome | None:
    ...
    def get_decisions_by_risk_level(
    ...
    def search_decisions(
        self, query: str, fields: list[str] = None, limit: int = 100
    ) -> list[DecisionOutcome]:
        """
    ...
    def export_decisions(
        ...
        decisions_to_export = (
            self._apply_filters(filters) if filters else list(self.decisions.values())
        )
    ...
    def _apply_filters(self, filters: dict[str, Any]) -> list[DecisionOutcome]:
        ...
        for key, value in filters.items():
            if key == "tool_name":
                ...
                filtered_decisions = [
                    d for d in filtered_decisions if d.supervisor_decision.risk_level == value
                ]
                ...
                filtered_decisions = [d for d in filtered_decisions if d.timestamp >= value]
            elif key == "end_time":
                filtered_decisions = [d for d in filtered_decisions if d.timestamp <= value]

        return filtered_decisions

    def _export_jsonl(self, decisions: list[DecisionOutcome], filepath: str | Path) -> str:
    ...
    def _export_json(self, decisions: list[DecisionOutcome], filepath: str | Path) -> str:
    ...
    def _export_csv(self, decisions: list[DecisionOutcome], filepath: str | Path) -> str:
    ...
    def _cleanup_memory(self):
        ...
        sorted_decisions = sorted(
            self.decisions.items(), key=lambda x: x[1].timestamp, reverse=True
        )
        ...
        self.decision_metadata = {
            k: v for k, v in self.decision_metadata.items() if k in self.decisions
        }
    ...
    def get_storage_info(self) -> dict[str, Any]:
        ...
        storage_info = {
            ...
            "parquet_file_size": (
                self.parquet_file.stat().st_size if self.parquet_file.exists() else 0
            ),
