...
from .schemas import (
    SupervisorDecision, RiskOfficerDecision, ToolProposal, DecisionContext,
    DecisionStatus, TradingMode
)
from .prompts import (
    SUPERVISOR_SYSTEM_PROMPT, SUPERVISOR_JSON_SCHEMA,
    RISK_OFFICER_SYSTEM_PROMPT, RISK_OFFICER_JSON_SCHEMA,
    format_supervisor_prompt, format_risk_officer_prompt
)
...
class BaseEvaluator:
    ...
    def _call_llm(self, system_prompt: str, user_prompt: str, json_schema: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            if field_name in response:
                ...
                if "minimum" in field_spec and isinstance(response[field_name], (int, float)):
                    if response[field_name] < field_spec["minimum"]:
                        raise ValueError(f"Value for {field_name} below minimum: {response[field_name]}")
                
                if "maximum" in field_spec and isinstance(response[field_name], (int, float)):
                    if response[field_name] > field_spec["maximum"]:
                        raise ValueError(f"Value for {field_name} above maximum: {response[field_name]}")
