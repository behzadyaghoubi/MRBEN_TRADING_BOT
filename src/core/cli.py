...
def run_smoke_test(minutes: int, symbol: str, logger: logging.Logger, enable_profiling: bool = False) -> int:
    ...
    try:
        ...
        try:
            ...
            if enable_profiling:
                ...
                # Save profile stats
                profile_file = f"artifacts/profile/smoke_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.prof"
...
def run_backtest(symbol: str, start_date: str, end_date: str, 
                config_path: Optional[str], logger: logging.Logger) -> int:
...
def run_live_trading(mode: str, symbol: str, config_path: Optional[str], 
...
def run_agent_operations(mode: str, symbol: str, config_path: Optional[str], 
                        halt: bool, regime_enabled: str, logger: logging.Logger) -> int:
        ...
        with MRBENAgentBridge(agent_config, trading_mode) as agent:
            ...
            if regime_enabled == "true":
                ...
                try:
                    ...
                    logger.info(f"📊 Feature Scores: ADX={regime_snapshot.scores.get('adx', 'N/A'):.1f}, "
            ...
            try:
                ...
                for i in range(60):  # Run for 60 seconds
                    ...
                    if i % 10 == 0:  # Log every 10 seconds
                        ...
                        logger.info(f"Agent running... Risk Level: {current_status['risk_gate_status']['risk_level']}")
                        ...
                        if regime_enabled == "true" and regime_update_counter % 3 == 0:
                            try:
                                ...
                                logger.info(f"🔄 Regime Update: {updated_regime.label.value.upper()} "
   ...
   %(prog)s agent --halt
    ...
    try:
        if args.command == "smoke":
            ...
            return run_backtest(args.symbol, args.start_date, args.end_date, 
                              args.config, logger)
        elif args.command == "live":
            return run_live_trading(args.mode, args.symbol, args.config, logger)
        elif args.command == "agent":
            return run_agent_operations(args.mode, args.symbol, args.config, args.halt, args.regime_enabled, logger)
        else:
