...
try:
    ...
    from ..utils.helpers import (
        _rolling_atr, _swing_extrema, _apply_soft_gate,
        enforce_min_distance_and_round
    )
    from ..utils.position_management import (
        _get_open_positions, _prune_trailing_registry, _count_open_positions
    )
    from ..utils.memory import log_memory_usage, cleanup_memory
...
class MT5LiveTrader:
    ...
    def _setup_logging(self) -> None:
        ...
        try:
            ...
            exists = any(isinstance(h, logging.handlers.RotatingFileHandler) and 
                        getattr(h, 'baseFilename', '') == os.path.abspath(self.config.LOG_FILE)
    ...
    def _initialize_component_state(self) -> None:
        ...
        self.max_spread_atr_frac = float(self.config.config_data.get("advanced", {}).get("dynamic_spread_atr_frac", 0.10))
        ...
        try:
            ...
            if MT5_AVAILABLE and self.mt5_connected:
                ...
                if not sym:
                    ...
                    self.logger.info(f"Preflight: stops_level={getattr(sym,'trade_stops_level',None)} freeze_level={getattr(sym,'trade_freeze_level',None)}")
    ...
    def _bootstrap_trailing(self) -> None:
        ...
        try:
            ...
            open_pos = _get_open_positions(self.config.SYMBOL, self.config.MAGIC, self.trailing_registry)
            self.trailing_registry = {}
            tick = mt5.symbol_info_tick(self.config.SYMBOL)
    ...
    def _trading_loop(self):
        ...
        while self.running:
            try:
                ...
                if not spread_ok:
                    self.logger.info(f"Spread too high: {spread_points:.1f} > {threshold:.1f}, waiting...")
                    time.sleep(self.config.SLEEP_SECONDS)
                    continue
                ...
                # Get open positions
                open_positions = _get_open_positions(self.config.SYMBOL, self.config.MAGIC, self.trailing_registry)
        ...
        try:
            ...
            if self.start_balance and self.start_balance > 0:
                ...
                if daily_loss > self.config.MAX_DAILY_LOSS:
                    self.logger.warning(f"Daily loss limit exceeded: {daily_loss:.2%} > {self.config.MAX_DAILY_LOSS:.2%}")
                    return True
            
            # Check daily trade count
            if hasattr(self, '_daily_trade_count') and self._daily_trade_count >= self.config.MAX_TRADES_PER_DAY:
    ...
    def _generate_trading_signal(self, df: pd.DataFrame, tick: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _should_execute_trade(self, signal_data: Dict[str, Any], open_count: int) -> bool:
        ...
        try:
            ...
            required = signal_data.get('required_consecutive', self.config.CONSECUTIVE_SIGNALS_REQUIRED)
            if self.consecutive_signals < required:
                self.consecutive_signals += 1
                self.logger.info(f"Building consecutive signals: {self.consecutive_signals}/{required}")
                return False
    ...
    def _execute_trade(self, signal_data: Dict[str, Any], df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if lot_size < self.config.MIN_LOT:
                self.logger.warning(f"Calculated lot size too small: {lot_size} < {self.config.MIN_LOT}")
                return False
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                self.logger.info(f"✅ {trade_type} order executed: {result.order} | "
                               f"Price: {entry_price:.2f} | Lot: {lot_size} | SL: {sl:.2f} | TP: {tp:.2f}")
                
                # Add trailing stop
                self.risk_manager.add_trailing_stop(
                    result.order, entry_price, sl, signal > 0
                )
                ...
                # Set cooldown
                self.cooldown_until = datetime.now() + timedelta(seconds=self.config.COOLDOWN_SECONDS)
                
                # Emit trade event
                if self.ev:
                    try:
                        self.ev.emit("trade_executed",
                                    signal=signal,
                ...
                self.logger.error(f"❌ Order failed: {getattr(result, 'retcode', 'Unknown')} | "
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                ...
                "reentry_lock_until": self.reentry_lock_until.isoformat() if self.reentry_lock_until else None,
            }
