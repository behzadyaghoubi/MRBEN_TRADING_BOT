...
class LabelEngineer:
    ...
    def _add_return_labels(self, labels_df: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            for period in self.return_periods:
                ...
                labels_df[f'return_{period}h_buy'] = (future_return > 0.001).astype(
                    int
                )  # 0.1% threshold
    ...
    def _add_confidence_labels(self, labels_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            labels_df['confidence_medium'] = (
                (labels_df['rule_confidence'] >= low_conf)
                & (labels_df['rule_confidence'] < high_conf)
            ).astype(int)
            ...
            if 'pa_avg_confidence' in labels_df.columns:
                labels_df['pa_confidence_low'] = (labels_df['pa_avg_confidence'] < low_conf).astype(
                    int
                )
                labels_df['pa_confidence_medium'] = (
                    (labels_df['pa_avg_confidence'] >= low_conf)
                    & (labels_df['pa_avg_confidence'] < high_conf)
                ).astype(int)
                labels_df['pa_confidence_high'] = (
                    labels_df['pa_avg_confidence'] >= high_conf
                ).astype(int)
    ...
    def _add_risk_adjusted_labels(self, labels_df: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'return_24h_value' in labels_df.columns and 'atr' in labels_df.columns:
                ...
                labels_df['sharpe_ratio'] = returns.rolling(20).mean() / (
            ...
            if 'return_24h_value' in labels_df.columns:
                ...
                labels_df['drawdown_severe'] = (labels_df['max_drawdown'] < -0.05).astype(
                    int
                )  # 5% threshold
    ...
    def get_label_names(self) -> list[str]:
        ...
        return [
            col
            for col in self.get_sample_labels().columns
            if 'signal_' in col or 'return_' in col or 'confidence_' in col
        ]
