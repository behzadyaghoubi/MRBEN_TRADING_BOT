...
class DatasetManager:
    ...
    def build_dataset(
        self, df: pd.DataFrame, features_df: pd.DataFrame, labels_df: pd.DataFrame
    ) -> dict:
        ...
        try:
            ...
            for split_name, split_data in splits.items():
                ...
                features_path = os.path.join(
                    self.features_dir, f'{split_name}_features_{timestamp}.parquet'
                )
                split_data['features'].to_parquet(features_path)

                # Save labels
                labels_path = os.path.join(
                    self.labels_dir, f'{split_name}_labels_{timestamp}.parquet'
                )
                split_data['labels'].to_parquet(labels_path)

                # Save complete data
                data_path = os.path.join(
                    self.datasets_dir, f'{split_name}_data_{timestamp}.parquet'
                )
    ...
    def _create_metadata(self, df: pd.DataFrame, splits: dict) -> dict:
        ...
        try:
            metadata = {
                ...
                'feature_count': len(
                    [col for col in df.columns if col not in self._get_label_columns(df)]
                ),
            ...
            metadata_path = os.path.join(
                self.datasets_dir,
                f'dataset_metadata_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json',
            )
    ...
    def load_dataset(
        self, split_name: str, timestamp: str = None
    ...
    def get_dataset_info(self) -> dict:
        ...
        try:
            info = {'available_splits': [], 'timestamps': [], 'file_sizes': {}, 'total_datasets': 0}
...
def build_dataset(
    df: pd.DataFrame, features_df: pd.DataFrame, labels_df: pd.DataFrame, config: dict
) -> dict:
