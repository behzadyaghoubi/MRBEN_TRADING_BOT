...
from .indicators import TechnicalIndicators, ema, rsi, atr, bollinger_bands
from .structure import MarketStructure, SwingPoint
from .pa import PriceActionValidator, PAResult, validate_price_action
...
class RuleDecision:
    ...
    pa_confirmation: Optional[List[PAResult]] = None  # Price Action confirmation
...
class RuleBasedStrategy:
    ...
    def _integrate_price_action(self, decision: RuleDecision, df: pd.DataFrame, 
                              structure: MarketStructure) -> RuleDecision:
        ...
        try:
            ...
            for pattern in pa_patterns:
                if (pattern.direction == "BULLISH" and decision.side == 1) or \
                   (pattern.direction == "BEARISH" and decision.side == -1) or \
            ...
            if relevant_patterns:
                ...
                decision.context['pa_confidence'] = np.mean([p.confidence for p in relevant_patterns])
    ...
    def _calculate_pa_enhanced_score(self, decision: RuleDecision, pa_patterns: List[PAResult]) -> float:
        """Calculate PA-enhanced score"""
        if not pa_patterns:
        ...
        enhanced_score = (decision.score * (1 - self.pa_weight) + 
                         pa_contribution * self.pa_weight)
    ...
    def _evaluate_trend_continuation(self, df: pd.DataFrame, structure: MarketStructure) -> RuleDecision:
        """
        ...
        trend_up = (structure.last_trend == 'UP' and 
                   structure.trend_strength > 0.6 and
                   ema_fast > ema_slow)
        
        trend_down = (structure.last_trend == 'DOWN' and 
                     structure.trend_strength > 0.6 and
                     ema_fast < ema_slow)
        ...
        if trend_up:
            ...
            pullback_to_ema = (abs(current_price - ema_fast) / current_atr < self.max_pullback_atr and
                              abs(current_price - ema_slow) / current_atr < self.max_pullback_atr)
    ...
    def _evaluate_breakout_retest(self, df: pd.DataFrame, structure: MarketStructure) -> RuleDecision:
        """
    ...
    def _check_bullish_rejection(self, df: pd.DataFrame) -> bool:
        ...
        for i in range(len(df) - 3, len(df)):
            ...
            if (lower_shadow > body_size * 2 and  # Long lower shadow
                close > open_price and  # Bullish close
                lower_shadow > upper_shadow):  # Lower shadow longer than upper
                return True
    ...
    def _check_bearish_rejection(self, df: pd.DataFrame) -> bool:
        ...
        for i in range(len(df) - 3, len(df)):
            ...
            if (upper_shadow > body_size * 2 and  # Long upper shadow
                close < open_price and  # Bearish close
                upper_shadow > lower_shadow):  # Upper shadow longer than lower
                return True
    ...
    def _check_liquidity_sweep(self, df: pd.DataFrame) -> Optional[Dict]:
        ...
        for i in range(len(df) - 5, len(df) - 1):
            ...
            if len(prev_highs) > 0 and len(prev_lows) > 0:
                ...
                elif (current_high > prev_highs.max() and 
                      current_close < prev_highs.max()):
                    return {
                        'type': 'BEARISH',
                        'index': i,
    ...
    def _check_bullish_reversal(self, df: pd.DataFrame) -> Optional[Dict]:
    ...
    def _check_bearish_reversal(self, df: pd.DataFrame) -> Optional[Dict]:
    ...
    def _calculate_tc_score(self, df: pd.DataFrame, structure: MarketStructure, trend: str) -> float:
    ...
    def _calculate_br_score(self, df: pd.DataFrame, structure: MarketStructure, breakout_type: str) -> float:
    ...
    def _calculate_rv_score(self, df: pd.DataFrame, structure: MarketStructure, 
                           reversal_type: str, rsi_divergence: bool) -> float:
        ...
        total_score = base_score + reversal_strength + divergence_bonus + quality_bonus + trend_exhaustion
        return min(max(total_score, 0.0), 1.0)
