...
class MarketStructureAnalyzer:
    ...
    def _calculate_swing_strength(
        self, high_series: pd.Series, low_series: pd.Series, index: int, swing_type: str
    ) -> float:
    ...
    def detect_bos(self, swings: list[SwingPoint], df: pd.DataFrame) -> dict | None:
        """
    ...
    def detect_choch(self, swings: list[SwingPoint], df: pd.DataFrame) -> dict | None:
        """
        ...
        for i in range(len(swings) - 2, 0, -1):
            ...
            if (
                prev_swing.type == 'low'
                and current_swing.type == 'high'
                and next_swing.type == 'low'
            ):
            ...
            # Bearish CHOCH: High -> Low -> High (but second high is lower)
                ...
                prev_swing.type == 'high'
                and current_swing.type == 'low'
                and next_swing.type == 'high'
            ):
                if next_swing.price < prev_swing.price:
                    return {
                        'type': 'BEARISH',
    ...
    def get_support_resistance_levels(
        self, swings: list[SwingPoint], current_price: float, lookback_bars: int = 100
    ) -> dict:
        """
        ...
        for low in lows:
            if low.price < current_price:
                support_levels.append(
                    {'price': low.price, 'strength': low.strength, 'index': low.index}
                )

        for high in highs:
            if high.price > current_price:
                resistance_levels.append(
                    {'price': high.price, 'strength': high.strength, 'index': high.index}
                )
...
def analyze_market_structure(
    df: pd.DataFrame, left_bars: int = 2, right_bars: int = 2
) -> MarketStructure:
    """
