...
def load_trade_log():
    if os.path.exists(TRADE_LOG):
        ...
        if "profit" not in df.columns and "exit_price" in df.columns and "entry_price" in df.columns:
    ...
    return dbc.Card(
        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    html.Label("Symbol:"),
                    dcc.Dropdown(SYMBOLS, value=settings.get("symbol", "XAUUSD"), id="symbol-select"),
                ], width=3),
                dbc.Col([
                    html.Label("Timeframe:"),
                    dcc.Dropdown(TIMEFRAMES, value=settings.get("timeframe", "M15"), id="timeframe-select"),
                ], width=3),
                dbc.Col([
                ...
                dbc.Col([
                    html.Label("Volume:"),
                    dcc.Input(type="number", value=settings.get("volume", 0.1), id="volume-input", min=0.01, step=0.01, style={"width": "100%"}),
                ], width=3),
            ], align="center", className="mb-2"),
            dbc.Button("Apply Settings", id="apply-settings", color="primary", className="me-2"),
...
def trade_log_table():
    return dash.dash_table.DataTable(
        ...
        style_cell={"fontFamily": "monospace", "fontSize": 13, "backgroundColor": "#2b2b2b", "color": "#f2f2f2"},
...
app.layout = dbc.Container([
    ...
    html.Footer([
        html.Small([
            ...
            " by MR BEN Team · ",
            html.A("GitHub", href="https://github.com/", target="_blank", className="text-info")
        ])
...
def save_settings(n, symbol, timeframe, enabled, volume):
    try:
        ...
        return dbc.Alert("Settings updated!", color="success", duration=2500), symbol, timeframe, enabled, volume
    except Exception as e:
        logging.error(f"Settings update failed: {e}")
        return dbc.Alert(f"Failed to update settings: {e}", color="danger", duration=4000), symbol, timeframe, enabled, volume
...
def update_dashboard(n):
    ...
    try:
        ...
        profit = float(last.get("balance", 0)) - float(df["balance"].iloc[0]) if "balance" in df.columns else "-"
        info = [
        ...
        max_drawdown = (df["balance"].cummax() - df["balance"]).max() if "balance" in df.columns else 0
        stats = [
        ...
        # Profit Curve
        x_axis = (
            df["timestamp"] if "timestamp" in df.columns else
            df["entry_time"] if "entry_time" in df.columns else
            df["time"] if "time" in df.columns else df.index
        )
        fig = go.Figure()
        if "balance" in df.columns:
            fig.add_trace(go.Scatter(x=x_axis, y=df["balance"], mode="lines+markers", name="Balance"))
        if "profit" in df.columns:
            fig.add_trace(go.Bar(x=x_axis, y=df["profit"], name="Profit", opacity=0.3))
        fig.update_layout(
            height=350, template="plotly_dark", margin=dict(t=40, l=10, r=10, b=20),
