...
class EventLogger:
    ...
    def log_trade_attempt(self, side: str, entry: float, sl: float, tp: float, 
                          volume: float, confidence: float, source: str):
        """Log trade attempt"""
        self.emit("trade_attempt", 
                  side=side, entry=entry, sl=sl, tp=tp,
                  volume=volume, confidence=confidence, source=source)
    
    def log_trade_execution(self, side: str, entry: float, sl: float, tp: float,
                           volume: float, order_id: int, execution_time: float):
        """Log successful trade execution"""
        self.emit("trade_execution",
                  side=side, entry=entry, sl=sl, tp=tp,
                  volume=volume, order_id=order_id, execution_time=execution_time)
...
class MFELogger:
    ...
    def log_tick(self, run_id: str, ticket: int, current_price: float, 
    ...
    def get_mfe_summary(self, ticket: int) -> Optional[Dict[str, Any]]:
