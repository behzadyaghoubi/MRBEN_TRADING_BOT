...
class PerformanceMonitor:
    ...
    def _collect_metrics(self):
        ...
        try:
            ...
            self.cpu_history.append({'timestamp': datetime.now(), 'cpu_percent': cpu_percent})
    ...
    def record_latency(self, operation: str, latency_ms: float):
        ...
        self.latency_history.append(
            {'timestamp': datetime.now(), 'operation': operation, 'latency_ms': latency_ms}
        )

        # Update running averages
        self.current_measurements[f'avg_latency_{operation}'] = (
            self.current_measurements[f'avg_latency_{operation}']
            * self.measurement_count[f'avg_latency_{operation}']
            + latency_ms
    ...
    def record_trade_latency(self, latency_ms: float):
        ...
        self.trade_latency_history.append({'timestamp': datetime.now(), 'latency_ms': latency_ms})
    ...
    def _get_memory_summary(self) -> dict[str, Any]:
        ...
        average = (
            sum(m['rss_mb'] for m in self.memory_history) / len(self.memory_history)
            if self.memory_history
            else 0
        )

        return {'current_rss': current, 'peak_rss': peak, 'average_rss': average}
    ...
    def _get_cpu_summary(self) -> dict[str, Any]:
        ...
        average = (
            sum(m['cpu_percent'] for m in self.cpu_history) / len(self.cpu_history)
            if self.cpu_history
            else 0
        )

        return {'current': current, 'peak': peak, 'average': average}
    ...
    def _get_latency_summary(self) -> dict[str, Any]:
        ...
        return {'total_operations': total_ops, 'average_latency': avg_latency}
    ...
    def get_recent_metrics(self, minutes: int = 5) -> dict[str, Any]:
        ...
        recent_latency = [m for m in self.latency_history if m['timestamp'] > cutoff_time]

        recent_memory = [m for m in self.memory_history if m['timestamp'] > cutoff_time]

        recent_cpu = [m for m in self.cpu_history if m['timestamp'] > cutoff_time]
