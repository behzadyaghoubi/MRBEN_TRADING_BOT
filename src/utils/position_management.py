...
def _get_open_positions(
    symbol: str, magic: int, trailing_registry: dict | None = None
) -> dict[int, Any]:
    """
...
def _prune_trailing_registry(
    trailing_registry: dict, open_pos_dict: dict, logger: logging.Logger
) -> int:
    """
...
def get_position_summary(positions: dict[int, Any]) -> dict[str, Any]:
    ...
    return {"count": len(positions), "total_volume": total_volume, "types": types}
