...
def log_memory_usage(logger: logging.Logger, context: str = "Memory check") -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"💻 System Memory: {system_used_mb:.1f}/{system_total_mb:.1f} MB ({system_percent:.1f}%)")
        except Exception:
            pass
...
def force_cleanup_if_needed(threshold_mb: float = 1000.0, logger: Optional[logging.Logger] = None) -> bool:
    """
    ...
    if check_memory_threshold(threshold_mb):
        if logger:
            logger.warning(f"🧹 High memory usage detected, forcing cleanup")
        cleanup_memory()
        return True
    return False
