...
class MT5Config:
    ...
    def _parse_credentials(self, raw: dict[str, Any]):
        ...
        if not self.DEMO_MODE and not (
            self.credentials.login and self.credentials.password and self.credentials.server
        ):
            raise RuntimeError(
    ...
    def _parse_session(self, raw: dict[str, Any]):
        ...
        self.session = SessionConfig(timezone=session_cfg.get("timezone", "Etc/UTC"))
