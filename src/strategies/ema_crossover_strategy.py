...
class EMACrossoverStrategy(BaseStrategy):
    ...
    def _analyze_ema_signals(self, df: pd.DataFrame, idx: int) -> str:
        ...
        # Bullish crossover
        bullish_cross = (
            current['ema_cross_change'] == 2 and  # Fast EMA crosses above slow EMA
            current['trend_strength'] > self.parameters['trend_strength_threshold'] and
            current['momentum'] > 0
        )
        
        # Bearish crossover
        bearish_cross = (
            current['ema_cross_change'] == -2 and  # Fast EMA crosses below slow EMA
            current['trend_strength'] > self.parameters['trend_strength_threshold'] and
            current['momentum'] < 0
        )
        ...
        # Trend continuation
        bullish_trend = (
            current['ema_cross'] == 1 and  # Fast EMA above slow EMA
            current['close'] > current['ema_fast'] and  # Price above fast EMA
            current['trend_strength'] > self.parameters['trend_strength_threshold'] * 0.5
        )
        
        bearish_trend = (
            current['ema_cross'] == -1 and  # Fast EMA below slow EMA
            current['close'] < current['ema_fast'] and  # Price below fast EMA
            current['trend_strength'] > self.parameters['trend_strength_threshold'] * 0.5
        )
