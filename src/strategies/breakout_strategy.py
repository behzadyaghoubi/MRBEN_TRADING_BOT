...
class BreakoutStrategy(BaseStrategy):
    ...
    def _calculate_breakout_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'tick_volume' in df.columns:
            ...
            df['volume_confirmation'] = df['tick_volume'] > df['volume_avg'] * self.parameters['volume_multiplier']
        else:
        ...
        df['false_breakout_up'] = (
            df['resistance_break'] & 
            (df['close'] < df['resistance_level'].shift(1) * (1 - self.parameters['false_breakout_threshold']))
        )
        
        df['false_breakout_down'] = (
            df['support_break'] & 
            (df['close'] > df['support_level'].shift(1) * (1 + self.parameters['false_breakout_threshold']))
        )
    ...
    def _analyze_breakout_signals(self, df: pd.DataFrame, idx: int) -> str:
        ...
        # Bullish breakout
        bullish_breakout = (
            current['resistance_break'] and
            current['volume_confirmation'] and
            not current['false_breakout_up'] and
            current['close'] > previous['close']
        )
        
        # Bearish breakout
        bearish_breakout = (
            current['support_break'] and
            current['volume_confirmation'] and
            not current['false_breakout_down'] and
            current['close'] < previous['close']
        )
        
        # Consolidation breakout
        consolidation_buy = (
            current['close'] > current['resistance_level'] and
            current['close'] > previous['close'] * 1.005 and  # Strong move
            current['volume_confirmation']
        )
        
        consolidation_sell = (
            current['close'] < current['support_level'] and
            current['close'] < previous['close'] * 0.995 and  # Strong move
            current['volume_confirmation']
        )
    ...
    def _calculate_breakout_confidence(self, df: pd.DataFrame, idx: int) -> float:
        ...
        if current['resistance_break']:
            level_strength = (current['close'] - current['resistance_level'].shift(1)) / current['resistance_level'].shift(1)
            confidence_factors.append(min(level_strength * 10, 0.3))
        
        if current['support_break']:
            level_strength = (current['support_level'].shift(1) - current['close']) / current['support_level'].shift(1)
            confidence_factors.append(min(level_strength * 10, 0.3))
