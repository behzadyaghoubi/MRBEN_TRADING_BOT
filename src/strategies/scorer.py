...
class AdaptiveScorer:
    ...
    def adapt_confidence(self, raw_conf: float, regime: RegimeSnapshot) -> AdaptiveDecision:
        ...
        if regime.session in self.config.session_filters:
            ...
            if regime.session not in ["london", "ny"]:
                ...
                logger.debug(
                    f"Non-optimal session {regime.session}: applying -{session_penalty} penalty"
                )
        ...
        regime_threshold = self.config.regime_thresholds.get(
            regime.label.value, self.config.base_conf
        )
    ...
    def _generate_notes(
        self, regime: RegimeSnapshot, adj_conf: float, threshold: float, allow_trade: bool
    ) -> str:
        ...
        if allow_trade:
            ...
            notes_parts.append(
                f"Trade blocked: {regime.label.value} regime requires {threshold:.2f} confidence"
            )
...
def create_adaptive_scorer(config_dict: dict | None = None) -> AdaptiveScorer:
    """
...
def quick_adapt_confidence(
    raw_conf: float, regime_label: str, config_dict: dict | None = None
) -> AdaptiveDecision:
    """
...
def adapt_confidence(
    raw_conf: float, regime_label: str | RegimeLabel, config_dict: dict | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        dummy_snapshot = RegimeSnapshot(
            label=regime_label, scores={}, session="unknown", ts=pd.Timestamp.now(), confidence=0.8
        )
