...
class BollingerStrategy(BaseStrategy):
    ...
    def _calculate_bollinger_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['bb_squeeze'] = (
            df['bb_width']
            < df['bb_width'].rolling(window=period).mean() * self.parameters['squeeze_threshold']
        )
    ...
    def _analyze_bollinger_signals(self, df: pd.DataFrame, idx: int) -> str:
        ...
        mean_reversion_buy = (
            current['bb_percent_b'] < 0.1  # Price near lower band
            and current['close'] > previous['close']  # Price starting to rise
            and not current['bb_squeeze']  # Not in squeeze
        )

        mean_reversion_sell = (
            current['bb_percent_b'] > 0.9  # Price near upper band
            and current['close'] < previous['close']  # Price starting to fall
            and not current['bb_squeeze']  # Not in squeeze
        )

        # Breakout signals
        breakout_buy = (
            current['close'] > current['bb_upper']  # Price breaks above upper band
            and current['close'] > previous['close']  # Strong momentum
            and current['bb_width'] > previous['bb_width']  # Expanding volatility
        )

        breakout_sell = (
            current['close'] < current['bb_lower']  # Price breaks below lower band
            and current['close'] < previous['close']  # Strong momentum
            and current['bb_width'] > previous['bb_width']  # Expanding volatility
        )
        ...
        # Squeeze breakout
        squeeze_buy = (
            current['bb_squeeze']  # In squeeze
            and current['close'] > current['bb_upper']  # Breaks out
            and current['bb_width'] > previous['bb_width'] * 1.2  # Significant expansion
        )

        squeeze_sell = (
            current['bb_squeeze']  # In squeeze
            and current['close'] < current['bb_lower']  # Breaks out
            and current['bb_width'] > previous['bb_width'] * 1.2  # Significant expansion
        )
    ...
    def _extract_features(self, df: pd.DataFrame, idx: int) -> dict[str, Any]:
        ...
        return {
            ...
            / df['close'].iloc[idx - 1],
            'bb_width_change': (
                (current['bb_width'] - df['bb_width'].iloc[idx - 1]) / df['bb_width'].iloc[idx - 1]
                if df['bb_width'].iloc[idx - 1] > 0
                else 0
            ),
        }
