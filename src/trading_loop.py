...
from .telemetry import PerformanceMetrics, MemoryMonitor
...
class TradingLoopManager:
    ...
    def _check_bar_gate(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            bar_ts_floor = bar_ts.replace(
                minute=(bar_ts.minute // tf_min) * tf_min, 
                second=0, 
                microsecond=0
            )
    ...
    def _log_cycle_summary(self, signal_data: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(f"   Last signal: {signal_data['signal']} (confidence: {signal_data['confidence']:.3f})")
