...
class TradingState:
    ...
    reentry_window_until: Optional[datetime] = None
    cooldown_until: Optional[datetime] = None
    last_trade_side: Optional[int] = None
...
class TradingSystem:
    ...
    def can_trade(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.state.cooldown_until and datetime.now() < self.state.cooldown_until:
                return False, f"Cooldown active until {self.state.cooldown_until.strftime('%H:%M:%S')}"
            ...
            if not self.risk_manager.can_open_new_trade(
                ...
                return False, f"Risk blocked new trade. open={open_count}/{self.config.MAX_OPEN_TRADES}"
    ...
    def validate_signal(self, signal_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.state.consecutive_signals < req_consec:
                return False, f"Consecutive {self.state.consecutive_signals} < required {req_consec}"
    ...
    def execute_trade(self, signal_data: Dict[str, Any], df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            cooldown_s = int(self.config.config_data.get("trading", {}).get("cooldown_seconds", 180))
