...
class PositionManager:
    ...
    def refresh_positions(self) -> None:
        ...
        try:
            ...
            # Update cache
            self.positions = {pos['ticket']: pos for pos in mt5_positions}
    ...
    def add_position(self, trade_signal) -> None:
        ...
        try:
            ...
            for ticket, position in self.positions.items():
                if (
                    position['symbol'] == trade_signal.symbol
                    and position['type'] == trade_signal.action
                    and abs(position['price_open'] - trade_signal.entry_price) < 0.001
                ):
    ...
    def close_position(self, ticket: int) -> bool:
        ...
        try:
            ...
            result = trade_executor.close_position(ticket, position['symbol'], position['volume'])
                ...
                if tick:
                    ...
                    if position['type'] == 'BUY':
                        position['unrealized_profit'] = (
                            (current_price - position['price_open']) * position['volume'] * 100000
                        )
                    else:
                        position['unrealized_profit'] = (
                            (position['price_open'] - current_price) * position['volume'] * 100000
                        )

        except Exception as e:
            self.logger.error(f"Error updating position prices: {e}")

    def get_total_exposure(self, symbol: str | None = None) -> dict[str, float]:
        """
        ...
        try:
            ...
            for position in self.positions.values():
                ...
                if symbol not in summary['symbols']:
                    summary['symbols'][symbol] = {'positions': 0, 'volume': 0.0, 'profit': 0.0}
            ...
            return {'total_positions': 0, 'symbols': {}, 'total_profit': 0.0, 'total_volume': 0.0}
