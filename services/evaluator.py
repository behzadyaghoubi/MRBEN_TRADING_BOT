...
class PerformanceEvaluator:
    ...
    def start_trade_tracking(self, trade_data: Dict) -> str:
        ...
        self.logger.info(f"Started tracking trade {trade_id}: {trade_data['side']} "
                        f"{trade_data['symbol']} @ {trade_data['entry_price']}")
        
        return trade_id
    
    def update_trade_price(self, trade_id: str, current_price: float, timestamp: Optional[datetime] = None):
    ...
    def _check_tp_sl_hits(self, trade_id: str, current_price: float):
        ...
        if trade["side"] == "BUY":
            ...
            # Check TP2
            if trade.get("tp2_price") and not trade["tp2_hit"] and current_price >= trade["tp2_price"]:
                trade["tp2_hit"] = True
    ...
    def close_trade(self, trade_id: str, exit_price: float, exit_reason: str, 
                   actual_pnl: Optional[float] = None) -> TradeMetrics:
        """
    ...
    def _update_performance_metrics(self, trade: TradeMetrics):
        ...
        if regime not in self.regime_metrics:
            self.regime_metrics[regime] = {
                ...
                "avg_confidence": 0.0, "avg_conformal": 0.0
            }
    ...
    def export_trade_data(self, filepath: str = None):
        ...
        if not filepath:
            filepath = f"logs/evaluation/trades_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
