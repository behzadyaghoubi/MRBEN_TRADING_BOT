...
class RiskGovernor:
    ...
    def _check_emergency_conditions(self) -> dict:
        ...
        if self.session_state["daily_pnl"] <= -abs(
            emergency_threshold * self.session_state["equity_start"]
        ):
            self.session_state["emergency_stop"] = True
            self.logger.critical("EMERGENCY STOP: Daily loss threshold breached!")
            return {
                "approved": False,
        ...
        total_loss = (self.session_state["equity_start"] - current_equity) / self.session_state[
            "equity_start"
        ]
    ...
    def _check_risk_limits(self, proposed_trade: dict, market_snapshot: dict) -> dict:
        ...
        if self.session_state["daily_pnl"] <= -abs(
            max_daily_loss * self.session_state["equity_start"]
        ):
            return {
                "approved": False,
    ...
    def _calculate_risk_assessment(self, proposed_trade: dict, market_snapshot: dict) -> dict:
        ...
        return {
            ...
            "equity_at_risk": proposed_trade.get("size_factor", 1.0)
            * self.config["risk"]["base_risk"],
