...
def test_model_comprehensive():
    ...
    print("🧪 Comprehensive Simple LSTM Model Test")
    print("=" * 60)
    ...
    try:
        ...
        high_conf_accuracy = np.mean(predicted_classes[high_conf_mask] == y_sequences[high_conf_mask])
        
        print(f"\nHigh confidence predictions (≥{high_conf_threshold}):")
        print(f"  Count: {np.sum(high_conf_mask)} ({np.sum(high_conf_mask)/len(predictions)*100:.1f}%)")
