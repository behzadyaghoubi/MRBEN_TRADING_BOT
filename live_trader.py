...
def log_trade(time, signal, price, sl, tp, result, buy_proba, sell_proba, features):
    ...
    headers = ['time', 'signal', 'price', 'sl', 'tp', 'result', 'buy_proba', 'sell_proba'] + [f'feature_{i}' for i in range(len(features))]
    ...
    with open(log_file, 'a', newline='', encoding='utf-8') as f:
        ...
        writer.writerow([time, signal, price, sl, tp, result, buy_proba, sell_proba] + list(features))
...
def dynamic_sl_tp_from_history(trade_log_path, current_price, signal, default_atr):
    ...
    if not os.path.exists(trade_log_path):
        return (current_price - default_atr * 1.5 if signal == 1 else current_price + default_atr * 1.5), \
    ...
    try:
        ...
        recent = df.tail(50)
        success_rate = len(recent[recent['result'] == 'WIN']) / len(recent) if len(recent) > 0 else 0.5
        ...
        sl = current_price - default_atr * sl_multiplier if signal == 1 else current_price + default_atr * sl_multiplier
        tp = current_price + default_atr * tp_multiplier if signal == 1 else current_price - default_atr * tp_multiplier
...
class TrailingStopManager(threading.Thread):
    ...
    def __init__(self, symbol, ticket, direction, trailing_distance, logger, stop_event, mt5_manager):
        super().__init__()
        self.symbol = symbol
        self.ticket = ticket
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                position = position[0]
                current_price = mt5.symbol_info_tick(self.symbol).bid if self.direction == 1 else mt5.symbol_info_tick(self.symbol).ask
...
class MT5Manager:
    ...
    def initialize(self):
        ...
        try:
            if not mt5.initialize(login=self.config.MT5_LOGIN, password=self.config.MT5_PASSWORD, server=self.config.MT5_SERVER):
    ...
    def get_latest_data(self):
        ...
        try:
            ...
            rates = mt5.copy_rates_from_pos(self.config.SYMBOL, self.config.TIMEFRAME, 0, self.config.BARS)
            if rates is None:
...
class LSTMModel:
    ...
    def load_model(self):
        ...
        try:
            if os.path.exists(self.config.LSTM_MODEL_PATH):
                ...
                self.logger.info(f"LSTM model loaded, built, and called successfully (classes: {self.num_classes})")
            else:
    ...
    def _prepare_features(self, df):
        ...
        try:
            ...
            df_window['bb_upper'], df_window['bb_lower'] = self._calculate_bollinger_bands(df_window['close'])
            ...
            df_window['upper_shadow'] = df_window['high'] - np.maximum(df_window['open'], df_window['close'])
            df_window['lower_shadow'] = np.minimum(df_window['open'], df_window['close']) - df_window['low']
            ...
            feature_columns = [
                ...
                'high_low_ratio', 'volume_ma', 'sma_50', 'ema_50',
                'stoch_k', 'stoch_d', 'williams_r', 'cci', 'volume_ratio'
            ]
            ...
            if len(available_columns) != self.config.NUM_FEATURES:
                self.logger.warning(f"Feature mismatch: Expected {self.config.NUM_FEATURES}, got {len(available_columns)}. Adjust model or features.")
...
class MLFilter:
    ...
    def load_model(self):
        ...
        try:
            if os.path.exists(self.config.ML_FILTER_MODEL_PATH):
                ...
                self.logger.warning(f"ML filter model not found: {self.config.ML_FILTER_MODEL_PATH}")
                return False
    ...
    def filter_signal(self, buy_proba, sell_proba, signal):
        ...
        try:
            ...
            features = np.array([buy_proba, sell_proba, signal, 0.0])  # Dummy 4th feature (change if you know the real one)
            ...
            if "shape mismatch" in str(ve):
                self.logger.warning(f"ML filter shape mismatch: {ve} - falling back to original signal")
                return signal
            else:
                raise
        except Exception as e:
            ...
            try:
                ...
                try:
                    if lstm_model is not None and lstm_scaler is not None:
                        ...
                        X_seq = lstm_scaler.transform(df_seq.values).reshape(1, lookback, len(feature_cols))
                    ...
                    # Example fallback:
                    signal, buy_proba, sell_proba = self.ml_filter.filter_signal(0.5, 0.5, 0), 0.5, 0.5
                ...
                if (filtered_signal != 0 and 
                    consecutive_signals >= 2 and  # Require 2 consecutive signals
                    price_action_confirmation(df)):
                    current_price = df['close'].iloc[-1]
                    atr = calculate_atr(df).iloc[-1]
                    # Calculate SL/TP
                    sl, tp = dynamic_sl_tp_from_history(
                        os.path.join(self.config.DATA_DIR, "trade_log.csv"),
                        current_price, filtered_signal, atr
                    )
                    # Send order
                    ...
                    if order_result:
                        ...
                        log_trade(
                            datetime.now(), filtered_signal, current_price,
                            sl, tp, "PENDING", buy_proba, sell_proba, features
                        )
                        self.logger.info(f"Trade executed: Signal={filtered_signal}, "
                                        f"Price={current_price:.2f}, SL={sl:.2f}, TP={tp:.2f}")
                        # Start trailing stop if order successful
                        if not self.config.SIMULATION_MODE:
                            trailing = TrailingStopManager(
                                self.config.SYMBOL, order_result.order, filtered_signal, atr * 1.5,
                                self.logger, self.stop_event, self.mt5_manager
                            )
                            trailing.start()
