...
def train_lstm():
    ...
    history = model.fit(
        X_train, y_train, epochs=50, batch_size=32, validation_data=(X_test, y_test), verbose=1
    )
    ...
    for i, (true, pred) in enumerate(zip(y_test[:10], predicted_classes, strict=False)):
