...
class TradingPerformanceAnalyzer:
    def __init__(self, trade_log_path: str = "data/trade_log_gold.csv", 
    ...
    def analyze_trades(self) -> Dict:
        ...
        analysis['avg_win'] = self.trades_df[self.trades_df['pnl'] > 0]['pnl'].mean() if analysis['winning_trades'] > 0 else 0
        analysis['avg_loss'] = self.trades_df[self.trades_df['pnl'] < 0]['pnl'].mean() if analysis['losing_trades'] > 0 else 0
        analysis['profit_factor'] = abs(analysis['avg_win'] / analysis['avg_loss']) if analysis['avg_loss'] != 0 else 0
        ...
        if 'R_post' in self.trades_df.columns:
            ...
            analysis['R_distribution'] = {
                ...
                '1.0-1.2': len(self.trades_df[(self.trades_df['R_post'] >= 1.0) & (self.trades_df['R_post'] < 1.2)]),
                '1.2-1.5': len(self.trades_df[(self.trades_df['R_post'] >= 1.2) & (self.trades_df['R_post'] < 1.5)]),
                '1.5-2.0': len(self.trades_df[(self.trades_df['R_post'] >= 1.5) & (self.trades_df['R_post'] < 2.0)]),
        ...
        if 'mfe' in self.trades_df.columns:
            ...
            analysis['mfe_distribution'] = {
                ...
                '0.5-0.8': len(self.trades_df[(self.trades_df['mfe'] >= 0.5) & (self.trades_df['mfe'] < 0.8)]),
                '0.8-1.0': len(self.trades_df[(self.trades_df['mfe'] >= 0.8) & (self.trades_df['mfe'] < 1.0)]),
    ...
    def analyze_events(self) -> Dict:
        ...
        if 'blocked_by_spread' in self.events_df['event'].values:
            ...
            analysis['spread_blocks'] = {
                ...
                'avg_spread': spread_blocks['spread'].mean() if 'spread' in spread_blocks.columns else 0
            }
        ...
        if 'order_attempt' in self.events_df['event'].values:
            ...
            analysis['order_attempts'] = {
                ...
                'avg_confidence': order_attempts['confidence'].mean() if 'confidence' in order_attempts.columns else 0
            }
        ...
        if 'conformal_rejected' in self.events_df['event'].values:
            ...
            analysis['conformal_rejects'] = {
                ...
                'avg_p_value': conformal_rejects['p_value'].mean() if 'p_value' in conformal_rejects.columns else 0
            }
    ...
    def plot_performance(self, save_path: str = "trading_analysis.png"):
        ...
        if 'pnl' in self.trades_df.columns:
            axes[0, 0].hist(self.trades_df['pnl'], bins=20, alpha=0.7, color='skyblue', edgecolor='black')
        ...
        if 'R_post' in self.trades_df.columns:
            axes[0, 1].hist(self.trades_df['R_post'], bins=15, alpha=0.7, color='lightgreen', edgecolor='black')
        ...
        # 3. MFE Distribution
        if 'mfe' in self.trades_df.columns:
            axes[1, 0].hist(self.trades_df['mfe'], bins=15, alpha=0.7, color='lightcoral', edgecolor='black')
        ...
        if 'entry_time' in self.trades_df.columns:
            try:
                ...
                axes[1, 1].plot(range(len(monthly_stats)), monthly_stats['win_rate'], 
                ...
                axes[1, 1].text(0.5, 0.5, f'Monthly analysis failed:\n{e}', 
    ...
    def generate_report(self) -> str:
        ...
        if trades_analysis:
            ...
            if 'split_tp_performance' in trades_analysis:
                ...
                report.append(f"Split TP Performance:")
        ...
        if events_analysis:
            ...
            if 'conformal_rejects' in events_analysis:
                ...
                report.append(f"Conformal Rejects: {conformal['count']} (Avg p-value: {conformal['avg_p_value']:.3f})")
