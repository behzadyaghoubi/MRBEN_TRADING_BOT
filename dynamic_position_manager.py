...
class DynamicPositionManager:
            ...
            for pos in positions:
                ...
                if ticket not in self.positions:
                    ...
                    if df is not None:
                        ...
                        print(f"📊 Position {ticket} opened | ATR: {self.positions[ticket]['atr_at_entry']:.2f}")
    ...
    def _get_market_data(self, bars: int = 100) -> Optional[pd.DataFrame]:
    ...
    def calculate_trailing_stop(self, position_info: Dict, current_atr: float) -> Optional[float]:
    ...
    def calculate_trailing_take_profit(self, position_info: Dict) -> Optional[float]:
            ...
            for ticket, position_info in self.positions.items():
                try:
                    ...
                    if new_sl is not None and abs(new_sl - current_sl) > 0.1:
                        print(f"📊 Position {ticket} | Updating SL: {current_sl:.2f} → {new_sl:.2f}")
                        self.modify_position(ticket, new_sl, current_tp)
                    
                    if new_tp is not None and abs(new_tp - current_tp) > 0.1:
                        print(f"📊 Position {ticket} | Updating TP: {current_tp:.2f} → {new_tp:.2f}")
    ...
    def run_management_loop(self, interval_seconds: int = 30):
        ...
        print(f"🚀 Starting Dynamic Position Manager")
            ...
            while True:
                try:
                    ...
                    if summary['total_positions'] > 0:
                        print(f"📊 Summary: {summary['total_positions']} positions | Total Profit: {summary['total_profit']:.2f}")
