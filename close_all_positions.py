...
def close_all_positions():
    ...
    print("-" * 80)
    print(f"{'Ticket':<10} {'Symbol':<12} {'Type':<6} {'Volume':<8} {'Price Open':<12} {'Price Current':<15} {'Profit':<12}")
    print("-" * 80)
    
    total_profit = 0
    for pos in positions:
        pos_type = "BUY" if pos.type == 0 else "SELL"
        print(f"{pos.ticket:<10} {pos.symbol:<12} {pos_type:<6} {pos.volume:<8.2f} {pos.price_open:<12.2f} {pos.price_current:<15.2f} {pos.profit:<12.2f}")
    ...
    for pos in positions:
        try:
            ...
            if result is None:
                ...
                print(f"❌ Failed to close position {pos.ticket}: retcode={result.retcode}, comment={result.comment}")
                failed_count += 1
    ...
    if remaining_positions is None:
        ...
        if remaining_count == 0:
            ...
            for pos in remaining_positions:
                pos_type = "BUY" if pos.type == 0 else "SELL"
                print(f"   {pos.ticket} | {pos.symbol} | {pos_type} | {pos.volume} | Profit: {pos.profit:.2f}")
