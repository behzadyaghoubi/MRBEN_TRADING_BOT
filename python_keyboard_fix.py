...
def fix_keyboard_permanently():
    ...
    try:
        ...
        # Set user language
        try:
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Control Panel\International\User Profile", 0, winreg.KEY_WRITE) as key:
        ...
        # Set system locale
        try:
            subprocess.run(['powershell', 'Set-WinSystemLocale', '-SystemLocale', 'en-US'], check=True)
        ...
        try:
            subprocess.run(['powershell', 'Set-WinUserLanguageList', '-LanguageList', 'en-US', '-Force'], check=True)
