...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('lstm_signal_balancer.log'), logging.StreamHandler()],
)
...
class LSTMSignalBalancerPro:
    ...
    def _generate_single_signal(
        ...
        if self.config.aggressive_mode:
            ...
            if (
                buy_prob >= aggressive_buy_threshold
            ...
            if (
                sell_prob >= aggressive_sell_threshold
                and sell_prob > hold_prob
                and sell_prob > buy_prob
            ):
        ...
        if (
            buy_prob > hold_prob
            and buy_prob > sell_prob
            and buy_prob >= (self.config.buy_threshold - 0.05)
        ):
            confidence = buy_prob
            strength = buy_prob - max(hold_prob, sell_prob)
            priority = 4
        ...
        if (
            sell_prob > hold_prob
            and sell_prob > buy_prob
            and sell_prob >= (self.config.sell_threshold - 0.05)
        ):
            confidence = sell_prob
            strength = sell_prob - max(hold_prob, buy_prob)
            priority = 4
    ...
    def plot_distribution(
        ...
        fig.suptitle(
            'LSTM Professional Signal Distribution Analysis', fontsize=16, fontweight='bold'
        )
...
def test_threshold_combinations_pro(df: pd.DataFrame) -> pd.DataFrame:
    ...
    test_configs = [
        SignalConfig(
            buy_threshold=0.12, sell_threshold=0.12, hold_threshold=0.75, aggressive_mode=True
        ),
        SignalConfig(
            buy_threshold=0.15, sell_threshold=0.15, hold_threshold=0.70, aggressive_mode=True
        ),
        SignalConfig(
            buy_threshold=0.18, sell_threshold=0.18, hold_threshold=0.65, aggressive_mode=True
        ),
        SignalConfig(
            buy_threshold=0.20, sell_threshold=0.20, hold_threshold=0.60, aggressive_mode=True
        ),
        SignalConfig(
            buy_threshold=0.15, sell_threshold=0.18, hold_threshold=0.68, aggressive_mode=True
        ),
        SignalConfig(
            buy_threshold=0.18, sell_threshold=0.15, hold_threshold=0.68, aggressive_mode=True
        ),
        SignalConfig(
            buy_threshold=0.10, sell_threshold=0.10, hold_threshold=0.80, aggressive_mode=False
        ),
        SignalConfig(
            buy_threshold=0.13, sell_threshold=0.13, hold_threshold=0.75, aggressive_mode=False
        ),
    ]
...
def main():
    ...
    print("   Output file: lstm_professional_signals_final.csv")
    print("   Distribution plot: lstm_professional_signals_distribution.png")
    print("   Test results: professional_threshold_testing_results.csv")
    print("   Log file: lstm_signal_balancer.log")
