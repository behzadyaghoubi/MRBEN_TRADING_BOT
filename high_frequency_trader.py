...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('high_frequency_trading.log'), logging.StreamHandler()],
)
...
class HighFrequencySignalGenerator:
    ...
    def generate_ultra_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in result_df.iterrows():
            ...
            signal, confidence, reason = self._generate_ultra_signal(
                buy_prob, hold_prob, sell_prob, row
            )
    ...
    def _generate_ultra_signal(
        self, buy_prob: float, hold_prob: float, sell_prob: float, row: pd.Series
    ) -> tuple[int, float, str]:
...
class HighFrequencyBacktester:
    ...
    def _calculate_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            / max(1, len(self.equity_curve) / 96),  # Assuming 96 15-min bars per day
        }
