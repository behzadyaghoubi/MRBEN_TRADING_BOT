...
class EnhancedSignalGenerator:
    ...
    def _apply_ml_filter(self, lstm_signal: int, lstm_conf: float, 
                        ta_signal: int, ta_conf: float) -> Tuple[int, float]:
...
class EnhancedLiveTrader:
        ...
        while self.running and not self.stop_event.is_set():
            try:
                ...
                if (signal_data['signal'] != 0 and 
                    ...
                    consecutive_signals >= self.config.CONSECUTIVE_SIGNALS_REQUIRED):
    ...
    def _get_simulation_data(self) -> Optional[pd.DataFrame]:
