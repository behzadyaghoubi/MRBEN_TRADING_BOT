        ...
        for i in range(num_tests):
            try:
                ...
                if signal_result:
                    ...
                    # Log every 10th test
                    if (i + 1) % 10 == 0:
                        logger.info(f"   Test {i+1}/{num_tests}: LSTM={lstm_signal}, TA={ta_signal}, ML={ml_signal}, Final={final_signal}")
                
            except Exception as e:
                logger.error(f"Signal generation error in test {i+1}: {e}")
                continue
        
        logger.info(f"✅ Signal generation test completed: {len(self.final_signals)} successful tests")
    ...
    def check_bias(self):
        ...
        for signal, percentage in percentages.items():
            if percentage > bias_threshold:
                logger.warning(f"⚠️ BIAS DETECTED: {signal} signals are {percentage:.1f}% (>{bias_threshold}%)")
                bias_detected = True
    ...
    def save_detailed_report(self):
        ...
        try:
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                ...
                for signal, percentage in percentages.items():
                    if percentage > bias_threshold:
                        f.write(f"⚠️ BIAS DETECTED: {signal} signals are {percentage:.1f}% (>{bias_threshold}%)\n")
                    else:
