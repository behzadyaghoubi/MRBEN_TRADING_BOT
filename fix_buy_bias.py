...
class BuyBiasFixer:
    ...
    def create_balanced_dataset(self, target_ratio=0.3):
        ...
        print(
            f"Original counts - HOLD: {len(hold_data)}, BUY: {len(buy_data)}, SELL: {len(sell_data)}"
        )
        ...
        self.balanced_data = self.balanced_data.sample(frac=1, random_state=42).reset_index(
            drop=True
        )
    ...
    def train_balanced_lstm(self, X_sequences, y_onehot):
        ...
        weight_dict = dict(zip(range(3), class_weights, strict=False))
