...
try:
    ...
    from services.risk_governor import ExecutionMode, RiskGovernor
...
class EnhancedSignalGenerator:
    ...
    def _technical_analysis(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            return {'signal': signal, 'confidence': confidence, 'score': avg_signal}
    ...
    def _combine_signals(self, lstm: dict, ml: dict, tech: dict) -> dict:
        ...
        # Ensemble signal
        weighted_signal = sum(s * w for s, w in zip(signals, weights, strict=False))
        weighted_confidence = sum(c * w for c, w in zip(confidences, weights, strict=False))
        ...
        return {
            ...
            'components': {'lstm': lstm, 'ml': ml, 'technical': tech},
        }
...
class EnhancedAILiveTrader:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.ai_mode = self.ai_control.get(
            "mode", "shadow"
        ).lower()  # "shadow" | "copilot" | "autopilot"
    ...
    def _init_layer2_brain(self):
        ...
        if ENHANCED_AI_AVAILABLE:
            ...
            if (
                self.policy_brain
                and hasattr(self.policy_brain, 'conformal_gate')
                and self.policy_brain.conformal_gate
            ):
                if (
                    hasattr(self.policy_brain.conformal_gate, 'conf')
                    and self.policy_brain.conformal_gate.conf
                ):
                    ...
                    self.logger.warning(
                        "⚠️ Conformal gate loaded but no threshold configuration found"
                    )
            else:
    ...
    def _init_layer3_governor(self):
        ...
        if ENHANCED_AI_AVAILABLE:
            ...
            self.logger.warning(
                "⚠️ Layer 3: RiskGovernor not available, using basic risk management"
            )
    ...
    def _log_system_config(self):
        ...
        self.logger.info(
            f"🧠 Layer 1: {'✅' if self.lstm_model else '❌'} LSTM + {'✅' if self.ml_filter else '❌'} ML Filter"
        )
        ...
        while self.running:
            try:
                ...
                if self.risk_governor and brain_decision["decision"] != "HOLD":
                    governor_result = self.risk_governor.validate_trade(
                        brain_decision, snapshot["market"]
                    )
                    ...
                    if not governor_result["approved"]:
                        ...
                        brain_decision = {
                            "decision": "HOLD",
                            "accept_reason": governor_result["reason"],
                        }
                ...
                if should_execute:
                    ...
                    if self.ai_mode == "shadow":
                        ...
                        tp_policy = brain_decision.get(
                            "tp_policy",
                            {"split": True, "tp1_r": 0.8, "tp2_r": 1.5, "tp1_share": 0.5},
                        )

                        if tp_policy.get("split", True):
                            tp1 = self._r_to_price(
                                entry_price,
                                sl_price,
                                layer1_signal['signal'],
                                tp_policy.get("tp1_r", 0.8),
                            )
                            tp2 = self._r_to_price(
                                entry_price,
                                sl_price,
                                layer1_signal['signal'],
                                max(tp_policy.get("tp2_r", 1.5), 1.2),
                            )
                            ...
                            tp_price = self._r_to_price(
                                entry_price, sl_price, layer1_signal['signal'], 1.5
                            )
                            self._log_shadow(
                                "BUY" if is_buy else "SELL",
                                entry_price,
                ...
                if (
                    datetime.now() - self.session_stats.get("last_stats_log", datetime.now())
                ).total_seconds() >= 600:  # هر 10 دقیقه
                    ...
                    self.logger.info(
                        f"[STATS] seen={seen} accept_rate={ar:.2f} risk_reject_rate={rr:.2f} would_exec_rate={wr:.2f}"
                    )
    ...
    def _build_market_snapshot(self, df: pd.DataFrame, layer1_signal: dict) -> dict:
        ...
        features = {
            ...
            "dow": float(
                last_row['time'].dayofweek if 'time' in df.columns else datetime.now().weekday()
            ),
        }
        ...
        # Market conditions
        market = {"spread": 30, "session": "london", "regime": regime}  # Simplified  # Simplified
        ...
        return {"features": features, "layer1": layer1_signal, "market": market, "context": context}
    ...
    def _log_ai_decision(self, layer1: dict, brain_decision: dict, snapshot: dict):
        ...
        signal_strength = (
            "🟢 STRONG"
            if layer1["confidence"] > 0.7
            else "🟡 MEDIUM" if layer1["confidence"] > 0.5 else "🔴 WEAK"
        )
            ...
            if len(positions) == 1:
                ...
                if need_be:
                    ...
                    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                        self.logger.info(
                            f"✅ BE applied to position #{p.ticket} at {entry_like:.2f}"
                        )
                    else:
                        self.logger.warning(
                            f"⚠️ Failed to apply BE to position #{p.ticket}: {result.comment if result else 'Unknown error'}"
                        )
    ...
    def _save_enhanced_trade_log(
        ...
        trade_log = {
            ...
            'execution_mode': (
                self.risk_governor.get_execution_mode().value if self.risk_governor else "unknown"
            ),
        }
    ...
    def _generate_session_report(self):
        ...
        if self.policy_brain:
            ...
            self.logger.info(
                f"🧠 Brain Acceptance Rate: {brain_stats.get('acceptance_rate', 0.0):.1%}"
            )
