...
def main():
    try:
        ...
        features = ['close', 'ret', 'sma_20', 'sma_50', 'atr', 'rsi', 'macd', 'macd_signal', 'hour', 'dow']
        output_df = df[['time', 'side', 'entry_price', 'sl_price', 'tp_price', 'label', 'r_outcome', 'regime'] + features].copy()
        
        # Fill NaN values
        for col in features:
            if col in output_df.columns:
                output_df[col] = output_df[col].fillna(method='ffill' if hasattr(pd.Series.fillna, 'method') else output_df[col].ffill())
