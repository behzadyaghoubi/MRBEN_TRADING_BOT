...
def load_price_series(symbol="XAUUSD.PRO", minutes=15, bars=1500):
    ...
    if px_df is None:
        ...
        if not os.path.exists(LOG_PATH):
            raise FileNotFoundError(f"No price data or trade log found")
...
def regime_from_row(row):
    ...
    try:
        ...
        if trend and above: return "UPTREND"
        if trend and not above: return "DOWNTREND"
        return "RANGE"
    except:
        return "RANGE"
        ...
        for _, tr in log.iterrows():
            try:
                ...
                if len(wi) < BARS_LOOKBACK: 
                    print(f"Skipping trade: insufficient lookback data ({len(wi)}/{BARS_LOOKBACK}) for time {t0}")
                    continue
                ...
                if fut.empty: 
                    ...
                    synthetic_times = pd.date_range(start=t0 + timedelta(minutes=1), 
                                                  periods=MAX_HORIZON_MIN, 
                                                  freq='1min')
                    
                    # Create synthetic price path with some randomness
                    np.random.seed(42)  # For reproducible results
                    price_changes = np.random.normal(0, 0.001, len(synthetic_times))  # Small random changes
                    ...
                    if fut.empty:
                        print(f"Failed to create valid synthetic data")
                        continue
                ...
                if hit_tp: 
                    ...
                    label = 0; r_out = maxfav/risk if risk>0 else 0.0
