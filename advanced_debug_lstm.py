...
def analyze_model_behavior():
    ...
    try:
        ...
        print(f"SELL probabilities - Mean: {np.mean(sell_probs):.4f}, Std: {np.std(sell_probs):.4f}")
        print(f"HOLD probabilities - Mean: {np.mean(hold_probs):.4f}, Std: {np.std(hold_probs):.4f}")
...
def test_model_with_extreme_inputs():
    ...
    try:
        ...
        for case_name, test_data in test_cases:
            ...
            print(f"  Probabilities: SELL={prediction[0]:.4f}, HOLD={prediction[1]:.4f}, BUY={prediction[2]:.4f}")
