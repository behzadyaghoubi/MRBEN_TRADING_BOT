...
class ADAUSDRobot:
    ...
    def execute_trade(self, action, price, time_str, reason=""):
        ...
        if action == "BUY" and self.position == 0:
            ...
            logger.info(
                f"📈 Close LONG: Entry=${self.entry_price:.4f}, Exit=${price:.4f}, PnL={pnl*100:.2f}%"
            )
            self.position = 0
            self.entry_price = 0
            self.entry_time = None
            ...
            logger.info(
                f"📉 Close SHORT: Entry=${self.entry_price:.4f}, Exit=${price:.4f}, PnL={pnl*100:.2f}%"
            )
            self.position = 0
            self.entry_price = 0
            self.entry_time = None
    ...
    def run_simulation(self, df, delay=0.1):
        ...
        for i in range(len(df)):
            ...
            if i % 100 == 0:
                logger.info(
                    f"⏰ {current_time} - Price: ${current_price:.4f} - Signal: {current_signal} - Capital: ${self.capital:,.2f}"
                )
    ...
    def print_results(self):
        ...
        print(
            f"📈 Total Return:    {((self.capital - self.initial_capital) / self.initial_capital) * 100:.2f}%"
        )
