...
class LSTMBalancedSignalGenerator:
    ...
    def __init__(self, thresholds: Optional[SignalThresholds] = None):
        ...
        logger.info(f"LSTM Balanced Signal Generator initialized with thresholds: "
    ...
    def _generate_single_signal(self, 
                               buy_prob: float, 
                               hold_prob: float, 
                               sell_prob: float) -> Tuple[int, float, str]:
        """
        ...
        if not (0 <= buy_prob <= 1 and 0 <= hold_prob <= 1 and 0 <= sell_prob <= 1):
            logger.warning(f"Invalid probabilities: BUY={buy_prob}, HOLD={hold_prob}, SELL={sell_prob}")
        ...
        if buy_prob >= self.thresholds.buy_threshold and sell_prob >= self.thresholds.sell_threshold:
    ...
    def analyze_signal_distribution(self, df: pd.DataFrame, signal_col: str = 'balanced_signal') -> Dict[str, Any]:
        """
    ...
    def plot_signal_distribution(self, 
                               df: pd.DataFrame, 
                               signal_col: str = 'balanced_signal',
    ...
    def export_signals(self, 
                      df: pd.DataFrame, 
                      output_path: str,
                      signal_col: str = 'balanced_signal') -> None:
        """
        ...
        export_columns = ['time', 'close', 'lstm_buy_proba', 'lstm_hold_proba', 
                         'lstm_sell_proba', signal_col, 'signal_confidence', 'signal_reason']
...
def test_threshold_combinations(df: pd.DataFrame, 
                              test_thresholds: List[SignalThresholds]) -> pd.DataFrame:
    """
...
def main():
    ...
    print(f"   Output file: lstm_balanced_signals_final.csv")
    print(f"   Distribution plot: lstm_balanced_signals_distribution.png")
