...
def demonstrate_strategy_usage():
    ...
    print(f"   Custom Strategy Signal: {custom_signal['signal']} (confidence: {custom_signal['confidence']:.2f})")
    ...
    if signal['signal'] == 'BUY' and signal['confidence'] >= 0.6:
        ...
        print(f"      └─ Stop loss distance: {abs(signal['entry_price'] - signal['stop_loss']):.5f}")
        print(f"      └─ Risk per pip: Calculate based on lot size")
...
def demonstrate_backtesting_scenario():
    ...
    if trades:
        ...
        for i, trade in enumerate(trades[-3:], 1):  # Show last 3 trades
            print(f"   {i}. {trade['signal']} at {trade['entry_price']:.5f} "
