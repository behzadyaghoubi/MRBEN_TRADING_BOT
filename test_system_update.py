...
"""
    ...
    for file_path, status in results['data_files'].items():
        if status.get('exists'):
            if 'rows' in status:
                report += f"- {file_path}: ✅ {status['rows']} rows, {len(status['columns'])} columns\n"
            else:
...
"""
    ...
    available_data = sum(1 for status in results['data_files'].values() if status.get('exists') and 'rows' in status)
