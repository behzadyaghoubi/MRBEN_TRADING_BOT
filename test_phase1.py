...
def test_system_components():
    ...
    try:
        from mrben.core.system_integrator import SystemIntegrator, SystemStatus
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            report.append(f"A/B Tracks seen?: {'Yes' if has_control and has_pro else 'No'} (control & pro)")
            
            # Extract key metrics
            mrben_lines = [line for line in metrics_content.split('\n') if line.startswith('mrben_')]
    ...
    if log_file.exists():
        try:
            ...
            recent_lines = [line for line in lines[-20:] if any(label in line for label in ensemble_labels)]
            if recent_lines:
