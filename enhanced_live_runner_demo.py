...
class EnhancedLiveRunnerDemo:
    ...
    def setup_trade_logging(self):
        ...
        if not os.path.exists(self.log_path):
            columns = ["time", "symbol", "signal", "price", "confidence", "sl", "tp", "volume", "demo_mode"]
        ...
        try:
            ...
            if signal_str != "HOLD" and self.should_execute_signal(filtered_signal, confidence):
                if self.demo_mode:
                    ...
                    sl, tp = self.risk_manager.calculate_dynamic_sl_tp(self.symbol, current_price, signal_str)
                    volume = 0.01  # Demo volume
                    
                    self.log_trade(signal_str, current_price, confidence, sl, tp, volume)
                    self.last_signal = filtered_signal
                    
                    self.logger.info(f"DEMO: Executed {signal_str} signal with confidence {confidence:.3f}")
                    self.logger.info(f"DEMO: Price: {current_price:.2f}, SL: {sl:.2f}, TP: {tp:.2f}")
                else:
                    # Execute real trade
