...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('lstm_signal_balancer_ultra.log'),
...
class LSTMSignalBalancerUltra:
    ...
    def __init__(self, config: Optional[UltraSignalConfig] = None):
        """
    ...
    def _generate_ultra_signal(self, buy_prob: float, hold_prob: float, sell_prob: float) -> Tuple[int, float, str, float, int, float]:
        """
        ...
        if buy_prob > sell_prob and buy_prob > hold_prob and buy_prob >= self.config.relative_strength_threshold:
            amplified_buy = buy_prob * self.config.signal_amplification
        ...
        if sell_prob > buy_prob and sell_prob > hold_prob and sell_prob >= self.config.relative_strength_threshold:
            amplified_sell = sell_prob * self.config.signal_amplification
            confidence = min(amplified_sell, 1.0)
            strength = sell_prob - max(hold_prob, buy_prob)
            priority = 3
            return -1, confidence, f"SELL_RELATIVE_{sell_prob:.3f}", strength, priority, amplified_sell
        
        # ULTRA Strategy 3: Competitive analysis with ultra-low thresholds
        if buy_prob >= (self.config.buy_threshold - 0.02) and sell_prob >= (self.config.sell_threshold - 0.02):
            if buy_prob > sell_prob:
                amplified_buy = buy_prob * self.config.signal_amplification
                confidence = min(amplified_buy - sell_prob + self.config.confidence_boost, 1.0)
                strength = buy_prob - sell_prob
                priority = 4
                return 1, confidence, f"BUY_COMPETITIVE_{buy_prob:.3f}", strength, priority, amplified_buy
            else:
                amplified_sell = sell_prob * self.config.signal_amplification
                confidence = min(amplified_sell - buy_prob + self.config.confidence_boost, 1.0)
                strength = sell_prob - buy_prob
                priority = 4
                return -1, confidence, f"SELL_COMPETITIVE_{sell_prob:.3f}", strength, priority, amplified_sell
        ...
        if hold_prob < 0.8:  # If HOLD is not very dominant
            if buy_prob > sell_prob and buy_prob >= 0.03:  # Extremely low threshold
                ...
                strength = buy_prob - sell_prob
                priority = 6
                return 1, confidence, f"BUY_FORCED_{buy_prob:.3f}", strength, priority, amplified_buy
            elif sell_prob > buy_prob and sell_prob >= 0.03:  # Extremely low threshold
                amplified_sell = sell_prob * self.config.signal_amplification
                confidence = min(amplified_sell, 1.0)
                strength = sell_prob - buy_prob
                priority = 6
                return -1, confidence, f"SELL_FORCED_{sell_prob:.3f}", strength, priority, amplified_sell
        ...
        if buy_prob > sell_prob and buy_prob >= 0.02:
            ...
            strength = buy_prob - sell_prob
            priority = 8
            return 1, confidence, f"BUY_LAST_RESORT_{buy_prob:.3f}", strength, priority, amplified_buy
        elif sell_prob > buy_prob and sell_prob >= 0.02:
            amplified_sell = sell_prob * self.config.signal_amplification
            confidence = min(amplified_sell, 1.0)
            strength = sell_prob - buy_prob
            priority = 8
            return -1, confidence, f"SELL_LAST_RESORT_{sell_prob:.3f}", strength, priority, amplified_sell
    ...
    def analyze_ultra_distribution(self, df: pd.DataFrame, signal_col: str = 'ultra_signal') -> Dict[str, Any]:
        """
    ...
    def plot_ultra_distribution(self, df: pd.DataFrame, signal_col: str = 'ultra_signal', save_path: str = None):
        """
    ...
    def export_ultra_signals(self, df: pd.DataFrame, output_path: str, signal_col: str = 'ultra_signal'):
        """
        ...
        export_columns = [
            ...
            'signal_priority', 'signal_amplified', 'signal_label', 'signal_category'
        ]
...
def test_ultra_threshold_combinations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    test_configs = [
        UltraSignalConfig(buy_threshold=0.05, sell_threshold=0.05, hold_threshold=0.90, signal_amplification=1.2),
        UltraSignalConfig(buy_threshold=0.08, sell_threshold=0.08, hold_threshold=0.85, signal_amplification=1.5),
        UltraSignalConfig(buy_threshold=0.10, sell_threshold=0.10, hold_threshold=0.80, signal_amplification=1.8),
        UltraSignalConfig(buy_threshold=0.12, sell_threshold=0.12, hold_threshold=0.75, signal_amplification=2.0),
        UltraSignalConfig(buy_threshold=0.08, sell_threshold=0.12, hold_threshold=0.82, signal_amplification=1.5),
        UltraSignalConfig(buy_threshold=0.12, sell_threshold=0.08, hold_threshold=0.82, signal_amplification=1.5),
        UltraSignalConfig(buy_threshold=0.03, sell_threshold=0.03, hold_threshold=0.95, signal_amplification=1.0),
        UltraSignalConfig(buy_threshold=0.15, sell_threshold=0.15, hold_threshold=0.70, signal_amplification=2.5),
    ]
...
def main():
    ...
    print(f"   Output file: lstm_ultra_signals_final.csv")
    print(f"   Distribution plot: lstm_ultra_signals_distribution.png")
    print(f"   Test results: ultra_threshold_testing_results.csv")
    print(f"   Log file: lstm_signal_balancer_ultra.log")
