...
for idx, row in trades.iterrows():
    ...
    if signal == 'BUY':
        open_trades.append({'open_time': time, 'direction': 'BUY', 'entry': price, 'symbol': symbol})
    elif signal == 'SELL':
        open_trades.append({'open_time': time, 'direction': 'SELL', 'entry': price, 'symbol': symbol})
...
if len(trades) > 0:
    ...
    for ot in open_trades:
        if ot['symbol'] == last_symbol:
            profit = (last_price - ot['entry']) if ot['direction'] == 'BUY' else (ot['entry'] - last_price)
            results.append({
                'open_time': ot['open_time'],
