...
class SignalPipelineDebugger:
    ...
    def debug_lstm_input_data(self):
        ...
        try:
            ...
            if len(available_features) < 7:
                logger.error(f"❌ Missing features: {set(required_features) - set(available_features)}")
                return False
            
            # Prepare data for LSTM
            data = df[available_features].values
            logger.info(f"📊 Raw data shape: {data.shape}")
            logger.info(f"📊 Data sample (last 5 rows):")
            logger.info(data[-5:])
            ...
            for i, feature in enumerate(available_features):
                ...
                logger.info(f"   {feature}: min={min_val:.4f}, max={max_val:.4f}, mean={mean_val:.4f}")
    ...
    def debug_lstm_prediction_step_by_step(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"📊 Scaled data sample (last 5 rows):")
            ...
            try:
                ...
                logger.info(f"📊 LSTM Analysis:")
                logger.info(f"   Signal class: {signal_class} ({signal_name})")
                logger.info(f"   Confidence: {confidence:.4f}")
                logger.info(f"   All probabilities: SELL={prediction[0][0]:.4f}, HOLD={prediction[0][1]:.4f}, BUY={prediction[0][2]:.4f}")
    ...
    def debug_technical_analysis(self):
        ...
        try:
            ...
            logger.info(f"📊 TA Signal Result:")
    ...
    def debug_ml_filter(self):
        ...
        try:
            ...
            logger.info(f"📊 ML Filter Result:")
    ...
    def debug_full_pipeline(self, num_tests=10):
        ...
        for i in range(num_tests):
            try:
                ...
                if signal_result:
                    ...
                    logger.info(f"   Test {i+1}: LSTM({result['lstm_signal']},{result['lstm_confidence']:.3f}) + "
        ...
        if results:
            ...
            logger.info(f"   LSTM confidence: avg={avg_lstm_conf:.3f}, min={min_lstm_conf:.3f}, max={max_lstm_conf:.3f}")
