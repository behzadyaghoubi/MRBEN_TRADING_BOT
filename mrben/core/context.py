...
class MarketContext:
    ...
    def get_dynamic_multipliers(self, atr_val: float, ts_utc: datetime = None) -> dict:
        ...
        regime_mult = getattr(self.config.confidence.dynamic.regime, 
                             context["regime"]["regime"].lower(), 1.0)
        session_mult = getattr(self.config.confidence.dynamic.session, 
                              context["session"]["session"], 1.0)
    ...
    def get_advanced_market_summary(self) -> Optional[Dict[str, Any]]:
        """
