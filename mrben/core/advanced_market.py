...
from .metricsx import observe_market_metric, observe_market_regime_change
...
class AdvancedMarketAnalyzer:
    ...
    def __init__(
        ...
        self.volatility_detector: Optional[IsolationForest] = None
        ...
        logger.bind(evt="MARKET").info("advanced_market_analyzer_initialized",
    ...
    def _load_config(self) -> None:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                logger.bind(evt="MARKET").info("advanced_market_config_loaded", config_path=self.config_path)
            else:
                logger.bind(evt="MARKET").info("advanced_market_config_not_found", config_path=self.config_path)
    ...
    def _create_default_models(self) -> None:
        ...
        try:
            ...
            self.volatility_detector = IsolationForest(
                contamination=0.1,
                random_state=42
            )
    ...
    def analyze_market_regime(
        ...
        try:
            with self._lock:
                ...
                logger.bind(evt="MARKET").info("market_regime_analysis_completed",
    ...
    def _detect_enhanced_regime(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> MarketRegimeType:
    ...
    def _analyze_volatility(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> float:
    ...
    def _analyze_trend_strength(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> float:
        ...
        try:
            ...
            sma20_slope = (sma20 - context.sma20) / context.sma20 if hasattr(context, 'sma20') else 0
            sma50_slope = (sma50 - context.sma50) / context.sma50 if hasattr(context, 'sma50') else 0
    ...
    def _analyze_momentum(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> float:
    ...
    def _identify_key_levels(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> Tuple[List[float], List[float]]:
    ...
    def _identify_pattern_indicators(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> List[str]:
        ...
        try:
            ...
            logger.bind(evt="MARKET").warning("pattern_indicators_identification_failed", error=str(e))
            return []
    
    def _analyze_timeframes(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if historical_data and len(historical_data) > 100:
                ...
                timeframe_analysis["1h"] = self._analyze_timeframe_data(historical_data, 4)  # 4x15min = 1h
                timeframe_analysis["4h"] = self._analyze_timeframe_data(historical_data, 16)  # 16x15min = 4h
    ...
    def _analyze_timeframe_data(self, data: List[Dict[str, Any]], aggregation_factor: int) -> Dict[str, Any]:
    ...
    def _extract_regime_features(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> Optional[List[float]]:
        ...
        try:
            with self._lock:
                ...
                summary = {
                    ...
                    "average_trend_strength": np.mean([r['trend_strength'] for r in recent_analyses]),
    ...
    def train_models(self, training_data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            logger.bind(evt="MARKET").info("starting_model_training", data_points=len(training_data))
            ...
            for data_point in training_data:
                features = self._extract_regime_features(
                    data_point['context'],
                    data_point.get('historical_data', [])
                )
            ...
            if len(X_regime) < 10:
                logger.bind(evt="MARKET").warning("insufficient_regime_training_data", samples=len(X_regime))
                return False
            ...
            logger.bind(evt="MARKET").info("model_training_completed",
