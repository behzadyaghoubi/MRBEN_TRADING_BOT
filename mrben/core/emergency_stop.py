...
class EmergencyStop:
    ...
    def __init__(self, 
        ...
        logger.bind(evt="EMERGENCY").info("emergency_stop_initialized",
    ...
    def _monitor_loop(self) -> None:
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                logger.bind(evt="EMERGENCY").error("monitoring_loop_error",
                                                 error=str(e))
                time.sleep(self.check_interval)
        ...
        try:
            ...
            if previous_state != self.state.is_active:
                if self.state.is_active:
                    logger.bind(evt="EMERGENCY").warning("emergency_stop_activated",
                                                       reason=self.state.trigger_reason,
                                                       source=self.state.trigger_source)
                else:
                    logger.bind(evt="EMERGENCY").info("emergency_stop_deactivated",
                                                    recovery_source=self.state.trigger_source)
                    
        except Exception as e:
            logger.bind(evt="EMERGENCY").error("emergency_state_check_error",
                                             error=str(e))
    ...
    def _trigger_emergency_stop(self, source: str, trigger_time: datetime) -> None:
        ...
        logger.bind(evt="EMERGENCY").critical("emergency_stop_triggered",
                                             source=source,
    ...
    def _trigger_recovery(self, source: str, recovery_time: datetime) -> None:
        ...
        if (self.auto_recovery and 
            self.state.triggered_at and
            (recovery_time - self.state.triggered_at).total_seconds() < self.recovery_delay):
            logger.bind(evt="EMERGENCY").info("recovery_delayed",
        ...
        logger.bind(evt="EMERGENCY").info("emergency_recovery_triggered",
                                         source=source,
    ...
    def _execute_emergency_callbacks(self) -> None:
        ...
        for callback in self.on_emergency_callbacks:
            try:
                ...
                logger.bind(evt="EMERGENCY").error("emergency_callback_error",
    ...
    def _execute_recovery_callbacks(self) -> None:
        ...
        for callback in self.on_recovery_callbacks:
            try:
                ...
                logger.bind(evt="EMERGENCY").error("recovery_callback_error",
                                                 callback=str(callback),
                                                 error=str(e))
    ...
    def manual_emergency_stop(self, reason: str = "Manual emergency stop") -> None:
        ...
        try:
            with open(self.halt_file_path, 'w') as f:
                ...
                f.write(f"Triggered at: {datetime.now(timezone.utc).isoformat()}\n")
                f.write(f"Source: manual\n")
            
            self._trigger_emergency_stop("manual", datetime.now(timezone.utc))
            
        except Exception as e:
            logger.bind(evt="EMERGENCY").error("manual_emergency_stop_failed",
                                             error=str(e))
    ...
    def manual_recovery(self) -> None:
        ...
        try:
            ...
            self._trigger_recovery("manual", datetime.now(timezone.utc))
            
        except Exception as e:
            logger.bind(evt="EMERGENCY").error("manual_recovery_failed",
                                             error=str(e))
    ...
    def get_halt_file_info(self) -> Optional[dict]:
        ...
        try:
            ...
            with open(self.halt_file_path, 'r') as f:
                content = f.read()
            ...
            logger.bind(evt="EMERGENCY").error("halt_file_info_error",
                                             error=str(e))
            return None
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            logger.bind(evt="EMERGENCY").error("cleanup_error",
                                             error=str(e))
