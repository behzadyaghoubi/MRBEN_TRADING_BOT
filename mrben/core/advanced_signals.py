...
from .metricsx import observe_signal_metric, observe_signal_quality
...
class AdvancedSignalGenerator:
    ...
    def __init__(
        ...
        self.fusion_model: Optional[RandomForestClassifier] = None
        ...
        logger.bind(evt="SIGNAL").info("advanced_signal_generator_initialized",
    ...
    def _load_config(self) -> None:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                logger.bind(evt="SIGNAL").info("advanced_signals_config_loaded", config_path=self.config_path)
            else:
                logger.bind(evt="SIGNAL").info("advanced_signals_config_not_found", config_path=self.config_path)
    ...
    def generate_trend_following_signal(
        ...
        try:
            ...
            logger.bind(evt="SIGNAL").error("trend_following_signal_generation_failed", error=str(e))
    ...
    def generate_breakout_signal(
        ...
        try:
            ...
            nearest_resistance = min([r for r in resistance_levels if r > price], default=float('inf'))
    ...
    def _weighted_average_fusion(self, signals: List[SignalComponent]) -> FusedSignal:
        ...
        try:
            ...
            weighted_direction = sum(s.direction * w for s, w in zip(signals, weights))
            ...
            strength = sum(s.strength * w for s, w in zip(signals, weights))
            confidence = sum(s.confidence * w for s, w in zip(signals, weights))
    ...
    def validate_signal(
        ...
        try:
            ...
            if not strength_valid:
                recommendations.append("Signal strength too low, consider waiting for stronger confirmation")
            ...
            consistency = len([d for d in directions if d == signal.direction]) / max(len(directions), 1)
            ...
            if quality_score < 0.6:
                recommendations.append("Signal quality below threshold, consider additional confirmation")
            if risk_score > 0.5:
                recommendations.append("High risk signal, reduce position size or wait for better conditions")
    ...
    def _calculate_trend_strength(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> float:
    ...
    def _calculate_trend_confidence(self, context: MarketContext, historical_data: Optional[List[Dict[str, Any]]]) -> float:
    ...
    def _extract_fusion_features(self, signals: List[SignalComponent]) -> Optional[List[float]]:
        ...
        try:
            with self._lock:
                ...
                for signal in recent_signals:
                    ...
                    summary["signal_type_distribution"][signal_type] = summary["signal_type_distribution"].get(signal_type, 0) + 1
                
                # Calculate averages
                if recent_signals:
                    summary["average_quality"] = np.mean([s.get('quality_score', 0.0) for s in recent_signals])
                    summary["average_confidence"] = np.mean([s.get('confidence', 0.0) for s in recent_signals])
    ...
    def train_models(self, training_data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            logger.bind(evt="SIGNAL").info("starting_signal_model_training", data_points=len(training_data))
            ...
            if len(X_signal) < 20:
                logger.bind(evt="SIGNAL").warning("insufficient_signal_training_data", samples=len(X_signal))
                return False
            ...
            logger.bind(evt="SIGNAL").info("signal_model_training_completed",
    ...
    def _extract_signal_features(self, data_point: Dict[str, Any]) -> Optional[List[float]]:
