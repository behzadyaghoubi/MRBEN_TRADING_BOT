...
class AgentState:
    ...
    last_decision_review: Optional[datetime] = None
    interventions_count: int = 0
    warnings_count: int = 0
    blocks_count: int = 0
    last_intervention: Optional[AgentIntervention] = None
...
class AgentBridge:
    ...
    def __init__(
        ...
        logger.bind(evt="AGENT").info("agent_bridge_initialized", 
    ...
    def _load_config(self) -> None:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                logger.bind(evt="AGENT").info("agent_config_not_found", config_path=self.config_path)
    ...
    def review_decision(
        ...
        try:
            with self._lock:
                ...
                self.state.last_decision_review = datetime.now(timezone.utc)
    ...
    def _detect_anomalies(
        ...
        if len(self.decision_history) > 10:
            ...
            buy_count = sum(1 for d in recent_decisions if d['decision']['action'] == 'ENTER' and d['decision']['dir'] == 1)
            sell_count = sum(1 for d in recent_decisions if d['decision']['action'] == 'ENTER' and d['decision']['dir'] == -1)
    ...
    def _execute_intervention(self, intervention: AgentIntervention) -> None:
        ...
        try:
            with self._lock:
                ...
                if intervention.action == AgentAction.INTERVENE:
                    for callback in self.on_intervention_callbacks:
                        try:
                            ...
                            logger.bind(evt="AGENT").error("intervention_callback_failed", 
                    ...
                    for callback in self.on_warning_callbacks:
                        try:
                            ...
                            logger.bind(evt="AGENT").error("warning_callback_failed", 
                                                        callback=str(callback), error=str(e))
                ...
                logger.bind(evt="AGENT").warning("agent_intervention_executed",
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "last_decision_review": self.state.last_decision_review.isoformat() if self.state.last_decision_review else None,
                ...
                "last_intervention": asdict(self.state.last_intervention) if self.state.last_intervention else None,
