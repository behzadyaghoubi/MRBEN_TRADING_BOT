...
from .metricsx import update_context, observe_block
...
class ABRunner:
    ...
    def __init__(self, ctx_factory: Callable, symbol: str, emergency_stop: Optional[EmergencyStop] = None, agent_bridge: Optional[AgentBridge] = None):
        ...
        logger.bind(evt="AB").info("ab_runner_initialized", 
                                  symbol=symbol,
    ...
    def on_bar(self, bar_data: dict) -> None:
        ...
        try:
            ...
            if self.emergency_stop and not self.emergency_stop.is_trading_allowed():
                logger.bind(evt="AB").warning("bar_processing_blocked_by_emergency",
                                           symbol=self.symbol)
                return
            ...
            if self.agent_bridge:
                ...
                if intervention:
                    logger.bind(evt="AB").warning("agent_intervention_detected",
            ...
            if self.emergency_stop and self.emergency_stop.is_trading_allowed():
                ...
                logger.bind(evt="AB").info("strategy_execution_skipped_emergency_stop",
                                         symbol=self.symbol)
            
        except Exception as e:
            logger.bind(evt="AB").error("bar_processing_error",
                                       symbol=self.symbol,
                                       error=str(e))
    ...
    def on_tick(self, tick_data: dict) -> None:
        ...
        try:
            ...
            if self.emergency_stop and not self.emergency_stop.is_trading_allowed():
                logger.bind(evt="AB").debug("tick_processing_blocked_by_emergency",
                                          symbol=self.symbol)
                return
            ...
            logger.bind(evt="AB").error("tick_processing_error",
                                       symbol=self.symbol,
                                       error=str(e))
    ...
    def _make_control_decision(self, ctx: MarketContext) -> DecisionCard:
        ...
        try:
            ...
            logger.bind(evt="AB").error("control_decision_error",
                                       symbol=self.symbol,
                                       error=str(e))
    ...
    def _make_pro_decision(self, ctx: MarketContext) -> DecisionCard:
        ...
        try:
            ...
            logger.bind(evt="AB").error("pro_decision_error",
                                       symbol=self.symbol,
                                       error=str(e))
            # Return safe HOLD decision
            return DecisionCard(
    ...
    def _log_decisions(self, dc_control: DecisionCard, dc_pro: DecisionCard) -> None:
        ...
        logger.bind(evt="AB").info("decisions_comparison",
                                  symbol=self.symbol,
    ...
    def _execute_control(self, dc: DecisionCard) -> None:
        ...
        if dc.action == "ENTER":
            ...
            logger.bind(evt="AB").info("control_position_opened",
                                      symbol=self.symbol,
            ...
            logger.bind(evt="AB").debug("control_decision_blocked",
                                       symbol=self.symbol,
                                       reason=dc.reason)
    ...
    def _execute_pro(self, dc: DecisionCard, ctx: MarketContext) -> None:
        ...
        if dc.action == "ENTER":
            ...
            logger.bind(evt="AB").info("pro_position_executed",
                                      symbol=self.symbol,
                                      direction="buy" if dc.dir > 0 else "sell",
                                      lot=dc.lot,
                                      entry=ctx.mid_price,
            ...
            logger.bind(evt="AB").debug("pro_decision_blocked",
                                       symbol=self.symbol,
                                       reason=dc.reason)
    ...
    def _simulate_pro_execution(self, dc: DecisionCard, ctx: MarketContext) -> None:
        ...
        if execution_success:
            logger.bind(evt="AB").info("pro_execution_simulated",
                                      symbol=self.symbol,
                                      track="pro",
                                      action="ENTER",
                                      direction="buy" if dc.dir > 0 else "sell")
        else:
            logger.bind(evt="AB").warning("pro_execution_failed",
                                         symbol=self.symbol,
                                         track="pro",
                                         action="ENTER")
    
    def _apply_agent_intervention(self, decision: DecisionCard, intervention: 'AgentIntervention') -> DecisionCard:
        ...
        try:
            ...
            logger.bind(evt="AB").info("agent_intervention_applied",
            ...
            logger.bind(evt="AB").error("agent_intervention_application_failed",
                                       error=str(e))
    ...
    def _update_metrics(self, ctx: MarketContext, dc_pro: DecisionCard) -> None:
        ...
        try:
            ...
            logger.bind(evt="AB").error("metrics_update_error",
                                       symbol=self.symbol,
                                       error=str(e))
    ...
    def get_statistics(self) -> dict:
        ...
        stats = {
            ...
            "pro": {
                "decisions": self.pro_decisions,
                "enters": self.pro_enters
            },
            "comparison": {
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            logger.bind(evt="AB").info("ab_runner_cleanup_complete",
                                      symbol=self.symbol)
        except Exception as e:
            logger.bind(evt="AB").error("cleanup_error",
                                       symbol=self.symbol,
                                       error=str(e))
    ...
    def reset_statistics(self) -> None:
        ...
        logger.bind(evt="AB").info("statistics_reset",
                                  symbol=self.symbol)
    
    def get_emergency_status(self) -> Optional[dict]:
