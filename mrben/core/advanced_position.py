...
from .typesx import DecisionCard, MarketContext, Levels
from .metricsx import observe_position_metric, observe_position_adjustment
...
class AdvancedPositionManager:
    ...
    def __init__(
        ...
        # ML models
        self.sizing_model: Optional[RandomForestRegressor] = None
        self.adjustment_model: Optional[RandomForestRegressor] = None
        ...
        self.risk_analytics: Optional[AdvancedRiskAnalytics] = None
        
        # Load configuration and models
        self._load_config()
        self._load_models()
        
        logger.bind(evt="POSITION").info("advanced_position_manager_initialized",
    ...
    def _load_config(self) -> None:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                logger.bind(evt="POSITION").info("advanced_position_config_loaded", config_path=self.config_path)
            else:
                logger.bind(evt="POSITION").info("advanced_position_config_not_found", config_path=self.config_path)
        except Exception as e:
            logger.bind(evt="POSITION").warning("advanced_position_config_load_failed", error=str(e))
        ...
        try:
            with self._lock:
                ...
                if strategy == PositionStrategy.FIXED_SIZE:
                    ...
                    lot_size = self._calculate_volatility_adjusted_size(decision, context, account_balance, risk_per_trade)
                elif strategy == PositionStrategy.ML_OPTIMIZED:
                    lot_size = self._calculate_ml_optimized_size(decision, context, account_balance, risk_per_trade)
                elif strategy == PositionStrategy.PORTFOLIO_OPTIMIZED:
                    lot_size = self._calculate_portfolio_optimized_size(decision, context, account_balance, risk_per_trade)
                else:
                    lot_size = self._calculate_fixed_size(account_balance, risk_per_trade)
                ...
                logger.bind(evt="POSITION").info("position_size_calculated",
    ...
    def _calculate_kelly_criterion(self, decision: DecisionCard, context: MarketContext, account_balance: float) -> float:
        ...
        try:
            ...
            if potential_loss > 0:
                kelly_fraction = (win_probability * potential_gain - loss_probability * potential_loss) / potential_gain
    ...
    def _calculate_volatility_adjusted_size(self, decision: DecisionCard, context: MarketContext, 
        ...
        try:
            ...
            logger.bind(evt="POSITION").warning("volatility_adjusted_size_calculation_failed", error=str(e))
            return 0.1  # Default 0.1 lot
    
    def _calculate_ml_optimized_size(self, decision: DecisionCard, context: MarketContext,
                                   account_balance: float, risk_per_trade: float) -> float:
        ...
        try:
            if not self.sizing_model or not self.scaler:
                ...
                return self._calculate_volatility_adjusted_size(decision, context, account_balance, risk_per_trade)
            ...
            logger.bind(evt="POSITION").warning("ml_optimized_size_calculation_failed", error=str(e))
            return self._calculate_volatility_adjusted_size(decision, context, account_balance, risk_per_trade)
    
    def _calculate_portfolio_optimized_size(self, decision: DecisionCard, context: MarketContext,
                                          account_balance: float, risk_per_trade: float) -> float:
        """Calculate position size optimized for portfolio"""
        try:
            # Start with ML optimized size
            base_size = self._calculate_ml_optimized_size(decision, context, account_balance, risk_per_trade)
            ...
            logger.bind(evt="POSITION").warning("portfolio_optimized_size_calculation_failed", error=str(e))
            return self._calculate_ml_optimized_size(decision, context, account_balance, risk_per_trade)
    
    def _apply_portfolio_constraints(self, lot_size: float, decision: DecisionCard, context: MarketContext) -> float:
        ...
        try:
            ...
            if total_exposure + lot_size > max_portfolio_exposure:
                ...
                logger.bind(evt="POSITION").warning("portfolio_exposure_limit_applied", 
            ...
            logger.bind(evt="POSITION").warning("portfolio_constraints_application_failed", error=str(e))
            return lot_size
    ...
    def _calculate_portfolio_factor(self, decision: DecisionCard, context: MarketContext) -> float:
        ...
        try:
            ...
            same_direction_count = sum(1 for pos in self.portfolio_positions.values() if pos.direction == decision.dir)
            if same_direction_count > 2:
                diversification_factor *= 0.8
            
            # Reduce size if too many positions in same session
            session_positions = sum(1 for pos in self.portfolio_positions.values() 
    ...
    def _calculate_correlation_factor(self, decision: DecisionCard, context: MarketContext) -> float:
        ...
        try:
            ...
            logger.bind(evt="POSITION").warning("correlation_factor_calculation_failed", error=str(e))
            return 1.0
    ...
    def _calculate_risk_reward_ratio(self, decision: DecisionCard, context: MarketContext) -> float:
        ...
        try:
            ...
            logger.bind(evt="POSITION").warning("risk_reward_ratio_calculation_failed", error=str(e))
            return 1.0
    
    def _calculate_max_loss(self, lot_size: float, decision: DecisionCard, context: MarketContext) -> float:
    ...
    def _calculate_sizing_factors(self, decision: DecisionCard, context: MarketContext, lot_size: float) -> Dict[str, float]:
        ...
        try:
            ...
            # Session factor
            session_factors = {"london": 1.0, "newyork": 1.0, "asia": 0.9, "overlap": 1.1, "weekend": 0.7}
    ...
    def _generate_sizing_recommendations(self, lot_size: float, risk_reward_ratio: float,
                                       sizing_factors: Dict[str, float], context: MarketContext) -> List[str]:
        ...
        try:
            ...
            if lot_size > 2.0:
                ...
                recommendations.append("Position size is very small - consider increasing if confidence is high")
            ...
            if risk_reward_ratio < 1.5:
                ...
                recommendations.append("Excellent risk-reward ratio - consider increasing position size")
            ...
            if context.spread_pts > 30:
                recommendations.append("Wide spreads detected - consider waiting for better conditions")
            ...
            logger.bind(evt="POSITION").warning("sizing_recommendations_generation_failed", error=str(e))
            return ["Position sizing analysis incomplete - proceed with caution"]
    
    def _calculate_sizing_confidence(self, decision: DecisionCard, context: MarketContext, strategy: PositionStrategy) -> float:
        ...
        try:
            ...
            logger.bind(evt="POSITION").warning("sizing_confidence_calculation_failed", error=str(e))
            return 0.5
    
    def _extract_sizing_features(self, decision: DecisionCard, context: MarketContext, account_balance: float) -> Optional[List[float]]:
    ...
    def _create_default_sizing_result(self, account_balance: float, risk_per_trade: float) -> PositionSizingResult:
    ...
    def add_portfolio_position(self, symbol: str, direction: int, lot_size: float,
        ...
        try:
            with self._lock:
                ...
                if self.portfolio_positions:
                    same_direction_count = sum(1 for pos in self.portfolio_positions.values() 
                ...
                logger.bind(evt="POSITION").info("portfolio_position_updated",
                                               symbol=symbol,
                                               lot_size=lot_size,
                                               pnl=unrealized_pnl)
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                long_positions = sum(1 for pos in self.portfolio_positions.values() if pos.direction > 0)
                short_positions = sum(1 for pos in self.portfolio_positions.values() if pos.direction < 0)
                
                # Calculate risk metrics
                total_risk = sum(pos.risk_exposure for pos in self.portfolio_positions.values())
                avg_correlation = np.mean([pos.correlation_score for pos in self.portfolio_positions.values()])
    ...
    def train_models(self, training_data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            logger.bind(evt="POSITION").info("starting_model_training", data_points=len(training_data))
            ...
            if len(X_sizing) < 10:
                logger.bind(evt="POSITION").warning("insufficient_sizing_training_data", samples=len(X_sizing))
                return False
            ...
            logger.bind(evt="POSITION").info("model_training_completed",
