...
from .metricsx import observe_trade_open, observe_trade_close
...
class PaperBroker:
    ...
    def __init__(self, symbol: str, track: str = "control"):
        self.symbol = symbol
        self.track = track
        self.pos: Optional[PaperPos] = None
        self.total_trades = 0
        self.wins = 0
        self.losses = 0
        self.breakevens = 0
        
        logger.bind(evt="PAPER").info("paper_broker_initialized", 
                                     symbol=symbol, track=track)
    
    def open(self, dc: DecisionCard) -> None:
        """Open a paper position based on decision card"""
        if self.pos is not None:
            logger.bind(evt="PAPER").warning("position_already_open", 
                                           symbol=self.symbol, track=self.track)
            return
        
        if dc.action != "ENTER":
            logger.bind(evt="PAPER").warning("invalid_action_for_open", 
                                           action=dc.action, track=self.track)
            return
        ...
        logger.bind(evt="PAPER").info("position_opened",
                                     symbol=self.symbol,
                                     track=self.track,
    ...
    def on_tick(self, bid: float, ask: float, atr_pts: float) -> None:
        ...
        if not p.tp1_hit and self._check_tp1(price):
            ...
            logger.bind(evt="PAPER").info("tp1_hit_breakeven",
                                         symbol=self.symbol,
                                         track=self.track,
                                         old_sl=old_sl,
                                         new_sl=p.entry)
    ...
    def _close_position(self, reason: str, r_multiple: float) -> None:
        ...
        logger.bind(evt="PAPER").info("position_closed",
                                     symbol=self.symbol,
                                     track=self.track,
                                     reason=reason,
                                     r_multiple=r_multiple,
