...
class ExposureGate:
    ...
    def evaluate(self, symbol: str, position_size: float, 
                 current_positions: Dict, account_balance: float) -> RiskGateResponse:
    ...
    def _check_daily_reset(self):
        ...
        if self.last_reset_date != today:
            ...
            try:
                ...
                reset_datetime = datetime.combine(today, reset_time).replace(tzinfo=timezone.utc)
                
                if now >= reset_datetime:
                    if self.last_reset_date is not None:  # Don't log on first run
                        logger.bind(evt="RISK").info("daily_pnl_reset", 
                                                   date=str(today),
                ...
                reset_datetime = datetime.combine(today, time(0, 0, 0)).replace(tzinfo=timezone.utc)
                if now >= reset_datetime:
                    self.daily_pnl = 0.0
                    self.last_reset_date = today
...
class ConsecutiveGate:
    ...
    def _cleanup_old_signals(self):
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=self.reset_time_hours)
        self.signal_history = [
            (ts, direction) for ts, direction in self.signal_history 
            if ts > cutoff_time
        ]
...
class CooldownGate:
    ...
    def record_loss(self, loss_amount: float):
        ...
        if loss_amount >= self.loss_threshold:
            ...
            logger.bind(evt="RISK").info("cooldown_triggered",
...
class RiskManager:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            self.logger.bind(evt="RISK").warning("advanced_risk_analytics_initialization_failed", error=str(e))
    ...
    def evaluate_all_gates(self, 
                          ...
                          account_balance: float) -> Tuple[bool, List[RiskGateResponse]]:
        """
        ...
        gates_to_evaluate = [
            ...
            ("exposure", lambda: self.exposure_gate.evaluate(symbol, position_size, current_positions, account_balance)),
        ...
        for gate_name, gate_func in gates_to_evaluate:
            try:
                ...
                if response.result == GateResult.REJECT:
                    rejected = True
                    self.logger.bind(evt="RISK").warning("gate_rejection",
                                                        gate=gate_name,
                                                        reason=response.reason,
                                                        data=response.data)
                elif response.result == GateResult.WARNING:
                    warnings.append(response)
                    self.logger.bind(evt="RISK").info("gate_warning",
                                                     gate=gate_name,
                                                     reason=response.reason,
                                                     data=response.data)
                ...
                responses.append(RiskGateResponse(
                    result=GateResult.REJECT,
                    gate_name=gate_name,
        ...
        if warnings and not rejected:
            self.logger.bind(evt="RISK").info("risk_warnings",
        ...
        # Log overall result
        if allowed:
            self.logger.bind(evt="RISK").info("risk_gates_passed",
                                            symbol=symbol,
                                            position_size=position_size,
                                            direction=signal_direction)
        else:
            rejections = [r for r in responses if r.result == GateResult.REJECT]
            self.logger.bind(evt="RISK").warning("risk_gates_rejected",
                                               symbol=symbol,
                                               position_size=position_size,
                                               direction=signal_direction,
    ...
    def record_trade_result(self, pnl: float, signal_direction: int):
        ...
        self.logger.bind(evt="RISK").info("trade_result_recorded",
                                        pnl=pnl,
                                        direction=signal_direction,
