...
class Decider:
    ...
    def __init__(self, cfg, mlf: MLFilter | None, lstm: LSTMDir | None):
        """
        ...
        try:
            ...
            self.logger.bind(evt="DECISION").warning(
                "advanced_signal_generator_initialization_failed", error=str(e)
            )
            self.advanced_signals = None

    def dynamic_conf(self, base: float, regime: str, session: str, dd_state: str) -> float:
        """
    ...
    def vote(self, rule_dir: int, pa_dir: int, ml_dir: int, lstm_dir: int) -> float:
        """
        ...
        v = w[0] * rule_dir + w[1] * pa_dir + w[2] * ml_dir + w[3] * lstm_dir
    ...
    def enhanced_vote(
        ...
        v = (
            ...
            + w[2] * ml_dir
            + w[3] * lstm_dir
            + w[4] * advanced_weighted
        )

        return abs(v)

    def decide(
        self, rule_dir: int, pa_dir: int, pa_score: float, market_data: dict, context: dict
    ) -> Decision:
        """
        ...
        if (
            self.cfg.strategy.price_action.enabled
            and pa_score < self.cfg.strategy.price_action.min_score
        ):
            return Decision("HOLD", 0, "pa_low_score", pa_score, 0.0)

        # 3) ML filter
        ml_dir, ml_p = 0, 0.0
        if self.cfg.strategy.ml_filter.enabled and self.mlf and self.mlf.model_loaded:
            try:
        ...
        if self.cfg.strategy.lstm_filter.enabled and self.lstm and self.lstm.model_loaded:
            try:
                x_seq = market_data.get('feature_seq', np.array([]))
                if len(x_seq) > 0:
                    lstm_dir, lstm_p = self.lstm.predict(x_seq)
                    if lstm_p < self.cfg.strategy.lstm_filter.agree_min or lstm_dir == -rule_dir:
        ...
        dyn_conf = self.dynamic_conf(self.cfg.confidence.base, regime, session, dd_state)
        ...
        if self.advanced_signals:
            try:
                ...
                fused_signal = self.advanced_signals.fuse_signals(
                    component_signals, method=SignalFusionMethod.WEIGHTED_AVERAGE
                )
                ...
                self.logger.bind(evt="DECISION").warning(
                    "advanced_signals_generation_failed", error=str(e)
                )

        # 6) Enhanced ensemble vote with advanced signals
        score = self.enhanced_vote(
            rule_dir, pa_dir, ml_dir, lstm_dir, advanced_signal_dir, advanced_signal_confidence
        )
