...
from .emergency_stop import EmergencyState, EmergencyStop
...
class TradingGuard:
    ...
    def _on_emergency_stop(self, state: EmergencyState) -> None:
        ...
        logger.bind(evt="GUARD").warning(
            "trading_operations_blocked", reason=state.trigger_reason, source=state.trigger_source
        )

    def _on_recovery(self, state: EmergencyState) -> None:
        """Handle recovery from emergency stop"""
        logger.bind(evt="GUARD").info(
            "trading_operations_resumed", recovery_source=state.trigger_source
        )
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    logger.bind(evt="GUARD").error(
                        "trading_operation_error", operation=operation_name, error=str(e)
                    )
                    raise
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    logger.bind(evt="GUARD").error(
                        "decision_operation_error", operation=operation_name, error=str(e)
                    )
                    raise
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    logger.bind(evt="GUARD").error(
                        "order_execution_error", operation=operation_name, error=str(e)
                    )
                    raise
    ...
    def get_last_blocked_operation(self) -> str | None:
    ...
    def get_status_summary(self) -> dict:
        ...
        return {
            ...
            "emergency_triggered_at": (
                emergency_state.triggered_at.isoformat() if emergency_state.triggered_at else None
            ),
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            if not guard.check_trading_allowed():
                logger.bind(evt="GUARD").warning(
                    "operation_requires_trading_allowed", operation=operation_name
                )
