...
class OrderFillingOptimizer:
    ...
    def __init__(self, config):
        ...
        self.default_mode = OrderFillingMode(
            getattr(config.order_management, 'default_filling_mode', 'ioc')
        )
    ...
    def select_filling_mode(
        self, symbol_info: dict, volume: float, volatility: float, urgency: str = "normal"
    ) -> OrderFillingMode:
        """
...
class SlippageManager:
    ...
    def calculate_slippage(
        self, requested_price: float, executed_price: float, order_type: OrderType
    ) -> float:
    ...
    def record_slippage(
        ...
        if abs(slippage) > self.slippage_tolerance:
            logger.bind(evt="ORDER").warning(
                "high_slippage_detected", symbol=symbol, slippage=slippage, volume=volume
            )
...
class OrderExecutor:
    ...
    def place_order(
        self, order_request: OrderRequest, volatility: float = 0.0, urgency: str = "normal"
    ) -> OrderResult:
        """
    ...
    def _prepare_mt5_request(
        self, order_request: OrderRequest, filling_mode: OrderFillingMode
    ) -> dict:
        """Prepare MT5 order request"""
        # Convert order type
    ...
    def _execute_mt5_order(
        self, mt5_request: dict, order_request: OrderRequest, symbol_info: dict
    ) -> OrderResult:
        ...
        try:
            ...
            self.logger.bind(evt="ORDER").info(
                "order_executed",
                symbol=order_request.symbol,
    ...
    def cancel_order(self, ticket: int) -> bool:
        ...
        try:
            request = {"action": self.mt5_connector.mt5.TRADE_ACTION_REMOVE, "order": ticket}
    ...
    def get_order_status(self, ticket: int) -> dict | None:
