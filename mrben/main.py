...
class MRBENSystem:
    ...
    def __init__(self, config_path: str = "config/config.yaml"):
        ...
        self.integrator: SystemIntegrator | None = None
        self.running = False
    ...
    def start(self, mode: str = "paper", symbol: str = "XAUUSD.PRO", ab: bool = False) -> bool:
        ...
        try:
            logger.info(
                f"Starting MR BEN Trading System in {mode} mode with symbol {symbol}, A/B testing: {ab}"
            )
            ...
            wait_time = 0
            while (
                self.integrator.status not in [SystemStatus.RUNNING, SystemStatus.ERROR]
        ...
        try:
            if self.integrator:
                return {
                    ...
                    "start_time": (
                        self.integrator.start_time.isoformat()
                        if self.integrator.start_time
                        else None
                    ),
...
Examples:
  ...
  python main.py daemon                  # Run in daemon mode
    ...
    parser.add_argument('--ab', '-a', action='store_true', help='Enable A/B testing')
