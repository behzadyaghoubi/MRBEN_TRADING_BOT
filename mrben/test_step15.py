...
def test_step15_advanced_signals():
    ...
    try:
        ...
        if Path(config_path).exists():
            ...
            config_valid = all(
                section in config.get("advanced_signals", {}) for section in required_sections
            )
            ...
            if config_valid:
                ...
                print(
                    f"  - Core settings: {len(config['advanced_signals']['core_settings'])} parameters"
                )
                print(f"  - ML models: {len(config['advanced_signals']['ml_models'])} models")
                print(
                    f"  - Signal generation: {len(config['advanced_signals']['signal_generation'])} types"
                )
            else:
    ...
    try:
        ...
        if (
            signal_generator.enable_ml
            and signal_generator.enable_fusion
            and signal_generator.enable_validation
        ):
        ...
        class MockConfig:
            ...
            class Confidence:
                ...
                class Dynamic:
                    ...
                    session = type(
                        'obj', (object,), {'asia': 0.9, 'london': 1.0, 'ny': 1.0, 'off': 0.5}
                    )()
                    drawdown = type(
                        'obj', (object,), {'calm': 1.0, 'mild_dd': 0.8, 'deep_dd': 0.5}
                    )()
    ...
    try:
        from core.metricsx import (
            observe_signal_fusion,
            observe_signal_metric,
            observe_signal_quality,
        )
