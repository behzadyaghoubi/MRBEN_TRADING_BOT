...
def test_step14():
    ...
    try:
        ...
        from core.advanced_market import (
            AdvancedMarketAnalyzer, MarketRegimeType, TimeframeType, PatternType,
            MarketRegimeAnalysis, MultiTimeframeAnalysis, PatternAnalysis
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            low_vol_context = MarketContext(
                price=1.1050, bid=1.1049, ask=1.1051, atr_pts=15.0,
                sma20=1.1040, sma50=1.1020, session="london", regime="LOW",
                equity=10000.0, balance=10000.0, spread_pts=15.0, open_positions=0
            )
            
            high_vol_context = MarketContext(
                price=1.1050, bid=1.1049, ask=1.1051, atr_pts=120.0,
                sma20=1.1040, sma50=1.1020, session="overlap", regime="HIGH",
                equity=10000.0, balance=10000.0, spread_pts=45.0, open_positions=2
            )
            ...
            strong_uptrend_context = MarketContext(
                price=1.1100, bid=1.1099, ask=1.1101, atr_pts=30.0,
                sma20=1.1080, sma50=1.1050, session="london", regime="NORMAL",
                equity=10000.0, balance=10000.0, spread_pts=20.0, open_positions=0
            )
            
            # Test weak trend
            weak_trend_context = MarketContext(
                price=1.1050, bid=1.1049, ask=1.1051, atr_pts=20.0,
                sma20=1.1045, sma50=1.1040, session="asia", regime="LOW",
                equity=10000.0, balance=10000.0, spread_pts=15.0, open_positions=0
            )
            ...
            print(f"✅ Trend Strength Analysis - Strong: {strong_trend:.3f}, Weak: {weak_trend:.3f}")
            
            # Test 7: Test Support/Resistance Levels
            print("\n🎯 Testing Support/Resistance Levels...")
            
            support_levels, resistance_levels = market_analyzer._identify_key_levels(test_context, None)
            ...
            print(f"✅ Support/Resistance Levels - Support: {len(support_levels)}, Resistance: {len(resistance_levels)}")
