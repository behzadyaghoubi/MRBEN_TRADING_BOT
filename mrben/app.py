...
from core.configx import load_config, cfg_to_json
from core.loggingx import setup_logging, log_cfg
...
def main():
    ...
    ap.add_argument("--emergency-stop", action="store_true", help="Test emergency stop functionality")
    ...
    if cfg.emergency_stop.enabled:
        ...
        logger.bind(evt="BOOT").info("emergency_stop_initialized",
    ...
    try:
        ...
        logger.bind(evt="BOOT").info("agent_bridge_initialized",
                                    intervention_enabled=True,
                                    risk_threshold=0.8)
    except Exception as e:
        logger.bind(evt="BOOT").warning("agent_bridge_initialization_failed",
                                       error=str(e))
        agent_bridge = None
...
def test_agent_supervision(agent_bridge: AgentBridge):
    ...
    if intervention:
        logger.bind(evt="AGENT").warning("unexpected_intervention_on_normal_decision",
    ...
    if intervention:
        logger.bind(evt="AGENT").info("risky_decision_intervention",
                                     action=intervention.action,
                                     reason=intervention.reason,
